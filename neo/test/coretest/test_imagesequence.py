    ...
    def test_wrong_dimensions(self):
        ...
        with self.assertRaises(ValueError):
            ImageSequence([[0, 1, 2, 4, 2], [0, 1, 2, 4, 5]],
                          sampling_rate=500 * pq.Hz, units='V', spatial_scale=1*pq.um)

    def test_t_start(self):
        seq = ImageSequence(self.data, sampling_rate=500 * pq.Hz, units='V',
                            t_start=250 * pq.ms, spatial_scale=1 * pq.um)
...
class TestMethodImageSequence(unittest.TestCase):
    ...
    def test_signal_from_region(self):
        ...
        seq = ImageSequence(self.data, units='V', sampling_rate=500 * pq.Hz, t_start=250 * pq.ms,
                            spatial_scale=1*pq.um)
        ...
        with self.assertRaises(ValueError):  # no pixels in region
            ImageSequence(self.data, units='V', sampling_rate=500 * pq.Hz,
                          spatial_scale=1*pq.um).signal_from_region(RectangularRegionOfInterest(1, 1, 0, 0))
        with self.assertRaises(ValueError):
            ImageSequence(self.data, units='V', sampling_rate=500 *
                          pq.Hz, spatial_scale=1*pq.um).signal_from_region()
