...
class TestAnalogSignalConstructor(unittest.TestCase):
    ...
    def test__create_inconsistent_sampling_rate_and_period_ValueError(self):
        data = np.arange(10.0) * pq.mV
        self.assertRaises(
            ValueError, AnalogSignal, data, sampling_rate=1 * pq.kHz, sampling_period=5 * pq.s
        )
            ...
            targ = (
                (
                    ...
                    % (
                        signal.shape[1],
                        signal.shape[0],
                        signal.units.dimensionality.unicode,
                        signal.dtype,
                    )
                )
                + ("annotations: %s\n" % signal.annotations)
...
class TestAnalogSignalArrayMethods(unittest.TestCase):
    ...
    def test__compliant(self):
        ...
        assert_arrays_equal(
            self.signal1.array_annotations["anno2"], np.array(["a", "b", "c", "d", "e"])
        )
    ...
    def test__time_slice_deepcopy_array_annotations(self):
        ...
        self.assertFalse(
            all(self.signal1.array_annotations["test0"] == result.array_annotations["test0"])
        )
        self.assertFalse(
            all(self.signal1.array_annotations["test1"] == result.array_annotations["test1"])
        )
        self.assertFalse(
            all(self.signal1.array_annotations["test2"] == result.array_annotations["test2"])
        )
    ...
    def test__slice_should_modify_linked_channelindex(self):
        ...
        signal = AnalogSignal(
            np.arange(n * 100.0).reshape(100, n), sampling_rate=1 * pq.kHz, units="mV", name="test"
        )
        self.assertEqual(signal.shape, (100, n))
        signal.channel_index = ChannelIndex(
            index=np.arange(n, dtype=int), channel_names=["channel{}".format(i) for i in range(n)]
        )
        ...
        assert_array_equal(
            odd_channels.channel_index.channel_names,
            ["channel{}".format(i) for i in range(1, n, 2)],
        )
        assert_array_equal(
            signal.channel_index.channel_names, ["channel{}".format(i) for i in range(n)]
        )
    ...
    def test_comparison_operators_in_1d(self):
        ...
        assert_array_equal(
            ...
            np.array([False, True, True, True, True, True, True, True, True, True, True]).reshape(
                -1, 1
            ),
        )
        assert_array_equal(
        ...
        assert_array_equal(
            ...
            np.array([True, True, True, True, True, True, True, True, True, True, True]).reshape(
                -1, 1
            ),
        )
    ...
    def test__comparison_as_indexing_double_trace(self):
        ...
        assert_array_equal(
            signal[signal < 10], np.array([[0, 2, 4, 6, 8], [1, 3, 5, 7, 9]]).T * pq.V
        )
    ...
    def test__indexing_keeps_order_across_time(self):
        ...
        assert_array_equal(
            signal[mask],
            np.array(
                [[0, 11, 2, 13, 4, 15, 26, 27, 18, 19], [40, 31, 22, 33, 14, 25, 46, 37, 28, 49]]
            )
            * pq.V,
        )
    ...
    def test_splice_1channel_with_copy(self):
        ...
        assert_array_equal(
            ...
            np.array([0.0, 5.0, 10.0, 100.0, 100.0, 100.0, 30.0, 35.0, 40.0, 45.0, 50.0]),
        )
    ...
    def test_downsample(self):
        ...
        for factor in factors:
            ...
            self.assertEqual(
                signal.shape[-1], result.shape[-1]
            )  # preserve number of recording traces
    ...
    def test_resample_less_samples(self):
        ...
        for sample_count in sample_counts:
            ...
            self.assertAlmostEqual(
                sample_count / signal.shape[0] * signal.sampling_rate, result.sampling_rate
            )
            # only comparing center values due to border effects
            np.testing.assert_allclose(desired[3:-3], result.magnitude[3:-3], rtol=0.05, atol=0.1)
    ...
    def test_resample_more_samples(self):
        ...
        desired = np.interp(
            np.arange(sample_count) / factor,
            np.arange(signal.shape[0]),
            signal.magnitude.flatten(),
        ).reshape(-1, 1)
        result = signal.resample(sample_count)
    ...
    def test_compare_resample_and_downsample(self):
        ...
        for sample_count in sample_counts:
            ...
            self.assertEqual(
                desired.shape[-1], result.shape[-1]
            )  # preserve number of recording traces
            self.assertAlmostEqual(desired.sampling_rate, result.sampling_rate)
            # only comparing center values due to border effects
            np.testing.assert_allclose(
                desired.magnitude[3:-3], result.magnitude[3:-3], rtol=0.05, atol=0.1
            )
    ...
    def test_rectify(self):
        ...
        signal = AnalogSignal(
            data * pq.mV, sampling_rate=30000 * pq.Hz, units=pq.mV, array_annotations=array_anno
        )
        ...
        assert_arrays_equal(
            rectified_signal.array_annotations["anno1"], target_signal.array_annotations["anno1"]
        )
        assert_arrays_equal(
            rectified_signal.array_annotations["anno2"], target_signal.array_annotations["anno2"]
        )
...
class TestAnalogSignalEquality(unittest.TestCase):
    def test__signals_with_different_data_complement_should_be_not_equal(self):
        signal1 = AnalogSignal(
            np.arange(55.0).reshape((11, 5)), units="mV", sampling_rate=1 * pq.kHz
        )
        signal2 = AnalogSignal(
            np.arange(55.0).reshape((11, 5)), units="mV", sampling_rate=2 * pq.kHz
        )
...
class TestAnalogSignalCombination(unittest.TestCase):
    ...
    def test__compliant(self):
        ...
        assert_arrays_equal(
            self.signal2.array_annotations["anno2"], np.array(["k", "l", "m", "n", "o"])
        )
        self.assertIsInstance(self.signal2.array_annotations, ArrayDict)
    ...
    def test__add_two_consistent_signals_should_preserve_data_complement(self):
        ...
        signal2 = AnalogSignal(
            data2quant, sampling_rate=1 * pq.kHz, array_annotations={"abc": [1]}
        )
        assert_neo_object_is_compliant(signal2)
    ...
    def test__add_signals_with_inconsistent_data_complement_ValueError(self):
        ...
        signal2 = AnalogSignal(
            np.arange(10.0), units="mV", t_start=100.0 * pq.ms, sampling_rate=0.5 * pq.kHz
        )
        assert_neo_object_is_compliant(signal2)
    ...
    def test_concatenate_padding_quantity(self):
        ...
        assert_array_equal(
            np.array([0, 1, 2, 3, -1e-3, -1e-3, -1e-3, 4, 5, 6]).reshape((-1, 1)), result.magnitude
        )
    ...
    def test_concatenate_array_annotations(self):
        ...
        data2 = np.arange(4, 8).reshape(2, 2)
        signal1 = AnalogSignal(
            data1 * pq.V, sampling_rate=1 * pq.Hz, array_annotations=array_anno1
        )
        signal2 = AnalogSignal(
            data2 * pq.V,
            sampling_rate=1 * pq.Hz,
    ...
    def test_concatenate_complex(self):
        ...
        for attr in signal1._necessary_attrs:
            self.assertEqual(
                getattr(signal1, attr[0], None), getattr(concatenated12, attr[0], None)
            )

        assert_array_equal(
            np.vstack((signal1.magnitude, signal2.magnitude)), concatenated12.magnitude
        )
    ...
    def test_concatenate_multi_signal(self):
        ...
        concatenated = signal1.concatenate(
            signal2, signal3, signal4, padding=-1 * pq.V, overwrite=True
        )
        for attr in signal1._necessary_attrs:
            self.assertEqual(getattr(signal1, attr[0], None), getattr(concatenated, attr[0], None))
        assert_arrays_equal(
            np.array([0, 1, 2, 30, 35, -1, -1, 40, 5, 6]).reshape((-1, 1)), concatenated.magnitude
        )
