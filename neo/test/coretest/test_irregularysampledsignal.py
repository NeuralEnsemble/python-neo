...
class Test__generate_datasets(unittest.TestCase):
    ...
    def test__get_fake_values(self):
        ...
        arr_ann = get_fake_value(
            "array_annotations", dict, seed=5, obj=IrregularlySampledSignal, n=1
        )
...
class TestIrregularlySampledSignalProperties(unittest.TestCase):
    def setUp(self):
        ...
        self.signals = [
            IrregularlySampledSignal(t, signal=D, testattr="test")
...
class TestIrregularlySampledSignalArrayMethods(unittest.TestCase):
    ...
    def test_comparison_operators(self):
        assert_array_equal(
            ...
            np.array([[False, False, False, False, False, True, True, True, True, True]]).T,
        )
        assert_array_equal(
            self.signal1 == 5 * pq.mV,
        ...
        assert_array_equal(
            ...
            np.array([[True, True, True, True, True, True, True, True, True, True]]).T,
        )
    ...
    def test_resample(self):
        ...
        for factor in factors:
            ...
            np.testing.assert_allclose(
                self.signal1.magnitude, result.magnitude[::factor], rtol=1e-7, atol=0
            )
...
class TestIrregularlySampledSignalCombination(unittest.TestCase):
    ...
    def test__add_signals_with_inconsistent_times_AssertionError(self):
        signal2 = IrregularlySampledSignal(
            self.time1quant * 2.0, signal=np.arange(10.0), units="mV"
        )
        assert_neo_object_is_compliant(signal2)
    ...
    def test__merge(self):
        ...
        assert_arrays_equal(
            merged12.array_annotations["anno1"], np.array([0, 1, 2, 3, 4, 5, 100, 101, 102])
        )
    ...
    def test_concatenate_simple(self):
        signal1 = IrregularlySampledSignal(
            signal=[0, 1, 2, 3] * pq.s, times=[1, 10, 11, 14] * pq.s
        )
    ...
    def test_concatenate_array_annotations(self):
        ...
        n2 = len(data2)
        signal1 = IrregularlySampledSignal(
            signal=data1 * pq.s, times=range(n1) * pq.s, array_annotations=array_anno1
        )
        signal2 = IrregularlySampledSignal(
...
class TestAnalogSignalFunctions(unittest.TestCase):
    def test__pickle(self):
        signal1 = IrregularlySampledSignal(
            np.arange(10.0) / 100 * pq.s, np.arange(10.0), units="mV"
        )
...
class TestIrregularlySampledSignalEquality(unittest.TestCase):
    def test__signals_with_different_times_should_be_not_equal(self):
        ...
        signal2 = IrregularlySampledSignal(
            np.arange(10.0) / 100 * pq.ms, np.arange(10.0), units="mV"
        )
