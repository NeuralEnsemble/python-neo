...
class TestChannelIndex(unittest.TestCase):
    ...
    def test_channelindex__init(self):
        chx = ChannelIndex(file_origin="temp.dat", index=np.array([1]))
        assert_neo_object_is_compliant(chx)
    ...
    def check_creation(self, chx):
        ...
        targ3 = get_fake_value("description", str, seed=seed + 5, obj=ChannelIndex)
    ...
    def test__merge(self):
        ...
        assert_same_sub_schema(
            self.sigarrs1a + self.sigarrs2, chx1a.analogsignals, exclude=["channel_index"]
        )
    ...
    def test__children(self):
        ...
        self.assertEqual(
            self.chx1._data_child_containers, ("analogsignals", "irregularlysampledsignals")
        )
        self.assertEqual(
        ...
        self.assertEqual(
            len(self.chx1.data_children_recur), 2 * self.nchildren + 1 * self.nchildren ** 2
        )
        self.assertEqual(len(self.chx1.container_children), 1 * self.nchildren)
        self.assertEqual(len(self.chx1.container_children_recur), 1 * self.nchildren)
        self.assertEqual(len(self.chx1.children), 3 * self.nchildren)
        self.assertEqual(
            len(self.chx1.children_recur), 3 * self.nchildren + 1 * self.nchildren ** 2
        )

        assert_same_sub_schema(
            list(self.chx1._single_children),
            self.units1a + self.sigarrs1a + self.irrsig1a,
            exclude=["channel_index"],
        )

        assert_same_sub_schema(
            list(self.chx1.data_children),
            self.sigarrs1a + self.irrsig1a,
            exclude=["channel_index"],
        )
        assert_same_sub_schema(
            list(self.chx1.data_children_recur),
            self.sigarrs1a + self.irrsig1a + self.trains1[:2] + self.trains1[2:],
            exclude=["channel_index"],
        )

        assert_same_sub_schema(
            list(self.chx1.children),
            self.sigarrs1a + self.irrsig1a + self.units1a,
            exclude=["channel_index"],
        )
        assert_same_sub_schema(
            list(self.chx1.children_recur),
            self.sigarrs1a + self.irrsig1a + self.trains1[:2] + self.trains1[2:] + self.units1a,
            exclude=["channel_index"],
        )
    ...
    def test__filter_single_annotation_obj_single(self):
        ...
        res4 = self.targobj.filter(j=1, objects=[SpikeTrain, ChannelIndex])
    ...
    def test__filter_single_attribute_norecur(self):
        ...
        res0 = self.targobj.filter(name=self.sigarrs1a[0].name, recursive=False)
        assert_same_sub_schema(res0, targ)
    ...
    def test__filter_single_attribute_nodata_norecur(self):
        targ = []
        res0 = self.targobj.filter(name=self.sigarrs1a[0].name, data=False, recursive=False)
        assert_same_sub_schema(res0, targ)
    ...
    def test__filterdata_multi_partres_annotation_annotation(self):
        ...
        targ = [self.trains1[0], self.trains1[2], self.units1[0]]
    ...
    def test__deepcopy(self):
        leaf_childconts = ("analogsignals", "irregularlysampledsignals")
