...
class TestEvent(unittest.TestCase):
    def setUp(self):
        ...
        self.seg = Segment()
        self.evt = Event(
            [0.1, 0.5, 1.1, 1.5, 1.7, 2.2, 2.9, 3.0, 3.1, 3.3] * pq.ms,
            name="test",
            description="tester",
    ...
    def test_Event_creation(self):
        ...
        assert_arrays_equal(
            evt.labels, np.array(["test event 1", "test event 2", "test event 3"], dtype="U")
        )
    ...
    def test_Event_creation_invalid_labels(self):
        self.assertRaises(ValueError, Event, [1.1, 1.5, 1.7] * pq.ms, labels=["A", "B"])

    def test_Event_creation_from_lists(self):
        evt = Event([1.1, 1.5, 1.7], ["test event 1", "test event 2", "test event 3"], units=pq.ms)
        assert_arrays_equal(evt.times, [1.1, 1.5, 1.7] * pq.ms)
        assert_arrays_equal(evt.labels, np.array(["test event 1", "test event 2", "test event 3"]))
    ...
    def test__time_slice_deepcopy_array_annotations(self):
        ...
        self.assertFalse(
            all(self.evt.array_annotations["test0"][5:8] == result.array_annotations["test0"])
        )
        self.assertFalse(
            all(self.evt.array_annotations["test1"][5:8] == result.array_annotations["test1"])
        )
        self.assertFalse(
            all(self.evt.array_annotations["test2"][5:8] == result.array_annotations["test2"])
        )
    ...
    def test_time_slice_differnt_units(self):
        ...
        evt = Event(
            [0.1, 0.5, 1.1, 1.5, 1.7, 2.2, 2.9, 3.1, 3.3] * pq.ms,
            name="test",
            description="tester",
            file_origin="test.file",
            test1=1,
    ...
    def test_Event_merge(self):
        ...
        evt1 = Event(
            [1.1, 1.5, 1.7] * pq.ms,
            labels=np.array(["test event 1 1", "test event 1 2", "test event 1 3"], dtype="U"),
            name="test",
        ...
        evt2 = Event(
            [2.1, 2.5, 2.7] * pq.us,
            labels=np.array(["test event 2 1", "test event 2 2", "test event 2 3"], dtype="U"),
            name="test",
    ...
    def test_set_labels(self):
        evt = Event([1.1, 1.5, 1.7] * pq.ms, labels=["A", "B", "C"])
    ...
    def test_to_epoch(self):
        ...
        event = Event(
            times=np.array([5.0, 12.0, 23.0, 45.0]),
            units="ms",
        ...
        odd_event = Event(
            times=np.array([5.0, 12.0, 23.0]), units="ms", labels=np.array(["A", "B", "C"])
        )
