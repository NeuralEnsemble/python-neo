...
class Test__generate_datasets(unittest.TestCase):
    ...
    def test__get_fake_values(self):
        ...
        signal = get_fake_value('signal', un.Quantity, seed=0, dim=2)
        sampling_rate = get_fake_value('sampling_rate', un.Quantity,
                                       seed=1, dim=0)
        t_start = get_fake_value('t_start', un.Quantity, seed=2, dim=0)
...
class TestAnalogSignalArrayConstructor(unittest.TestCase):
    ...
    def test__create_from_numpy_array(self):
        ...
        self.assertEqual(signal[9, 0], 0.018*un.mV)
        self.assertEqual(signal[9, 1], 19*un.uV)

    def test__create_from_quantities_array(self):
        data = np.arange(20.0).reshape((10, 2)) * un.mV
        rate = 5000*un.Hz
        signal = AnalogSignal(data, sampling_rate=rate)
        assert_neo_object_is_compliant(signal)
        self.assertEqual(signal.t_start, 0*un.ms)
        self.assertEqual(signal.t_stop, data.shape[0]/rate)
        self.assertEqual(signal[9, 0], 18000*un.uV)
    ...
    def test__create_with_copy_false_should_return_view(self):
        ...
        assert_neo_object_is_compliant(signal)
        data[3, 0] = 99*un.mV
        self.assertEqual(signal[3, 0], 99000*un.uV)
...
class TestAnalogSignalArrayProperties(unittest.TestCase):
    def setUp(self):
        ...
        self.data = [np.arange(10.0).reshape((5, 2))*un.nA,
                     np.arange(-100.0, 100.0, 10.0).reshape((4, 5))*un.mV,
                     np.random.uniform(size=(100, 4))*un.uV]
...
class TestAnalogSignalArrayArrayMethods(unittest.TestCase):
    def setUp(self):
        ...
        self.data2quant = self.data2 * un.mV
        self.signal2 = AnalogSignal(self.data2quant,
                                         sampling_rate=1.0*un.Hz,
                                         name='spam', description='eggs',
                                         file_origin='testfile.txt',
                                         arg1='test')
    ...
    def test__index_dim0_should_return_quantity_array(self):
        ...
        self.assertIsInstance(result, un.Quantity)
        ...
        self.assertEqual(result.units, un.nA)
    ...
    def test__getitem_should_return_single_quantity(self):
        ...
        self.assertEqual(self.signal1[9, 3], 48000*un.pA)
    ...
    def test_comparison_operators(self):
        assert_arrays_equal(self.signal1[0:3, 0:3] >= 5*un.nA,
                            np.array([[False, False, False],
                                      [True, True, True],
                                      [True, True, True]]))
        assert_arrays_equal(self.signal1[0:3, 0:3] >= 5*un.pA,
    ...
    def test__simple_statistics(self):
        self.assertEqual(self.signal1.max(), 54000*un.pA)
        self.assertEqual(self.signal1.min(), 0*un.nA)
        self.assertEqual(self.signal1.mean(), 27*un.nA)
        self.assertEqual(self.signal1.std(), self.signal1.magnitude.std()*un.nA)
        self.assertEqual(self.signal1.var(), self.signal1.magnitude.var()*un.nA**2)
    ...
    def test__time_slice(self):
        ...
        targ = AnalogSignal(np.array([[2., 3.], [2., 3.]]).T,
                                 sampling_rate=1.0*un.Hz, units='mV',
                                 t_start=t_start,
                                 name='spam', description='eggs',
                                 file_origin='testfile.txt', arg1='test')
        assert_neo_object_is_compliant(result)
    ...
    def test__time_slice__offset(self):
        self.signal2.t_start = 10.0 * un.s
        assert_neo_object_is_compliant(self.signal2)
        ...
        self.assertEqual(self.signal2.t_start, 10.0 * un.s)
        self.assertEqual(result.t_stop, t_stop)
        self.assertEqual(result.t_start, t_start)
        self.assertEqual(result.sampling_rate, targ.sampling_rate)
    ...
    def test__time_slice__different_units(self):
        self.signal2.t_start = 10.0 * un.ms
        assert_neo_object_is_compliant(self.signal2)

        t_start = 2 * un.s + 10.0 * un.ms
        t_stop = 4 * un.s + 10.0 * un.ms
        ...
        assert_neo_object_is_compliant(self.signal2)
        self.assertEqual(self.signal2.t_start, 10.0 * un.ms)
        self.assertAlmostEqual(result.t_stop, t_stop, delta=1e-12*un.ms)
        self.assertAlmostEqual(result.t_start, t_start, delta=1e-12*un.ms)
        assert_arrays_almost_equal(result.times, targ.times, 1e-12*un.ms)
        self.assertEqual(result.sampling_rate, targ.sampling_rate)
        assert_arrays_equal(result, targ)
        assert_same_sub_schema(result, targ)
    ...
    def test__time_slice__no_explicit_time(self):
        ...
        t1 = 2 * un.s + 10.0 * un.ms
        t2 = 4 * un.s + 10.0 * un.ms
        ...
        for t_start,t_stop in [(t1,None),(None,None),(None,t2)]:
            ...
            targ = AnalogSignal(targ_array,
                                     t_start=t_start_targ.rescale(un.ms),
                                     sampling_rate=1.0*un.Hz, units='mV',
                                     name='spam', description='eggs',
                                     file_origin='testfile.txt', arg1='test')
            assert_neo_object_is_compliant(result)
            ...
            assert_neo_object_is_compliant(self.signal2)
            self.assertEqual(self.signal2.t_start, 10.0 * un.ms)
            self.assertAlmostEqual(result.t_stop, t_stop_targ, delta=1e-12*un.ms)
            self.assertAlmostEqual(result.t_start, t_start_targ, delta=1e-12*un.ms)
            assert_arrays_almost_equal(result.times, targ.times, 1e-12*un.ms)
            self.assertEqual(result.sampling_rate, targ.sampling_rate)
...
class TestAnalogSignalArrayEquality(unittest.TestCase):
    def test__signals_with_different_data_complement_should_be_not_equal(self):
            signal1 = AnalogSignal(np.arange(55.0).reshape((11, 5)),
                                        units="mV", sampling_rate=1*un.kHz)
            signal2 = AnalogSignal(np.arange(55.0).reshape((11, 5)),
                                        units="mV", sampling_rate=2*un.kHz)
...
class TestAnalogSignalArrayCombination(unittest.TestCase):
    ...
    def test__add_const_quantity_should_preserve_data_complement(self):
        ...
        self.assertEqual(self.signal1[0, 4], 4*un.mV)
        self.assertEqual(result[0, 4], 69000*un.uV)
    ...
    def test__add_signals_with_inconsistent_data_complement_ValueError(self):
        self.signal2.sampling_rate = 0.5*un.kHz
        assert_neo_object_is_compliant(self.signal2)
    ...
    def test__merge(self):
        ...
        data3 = np.arange(1000.0, 1066.0).reshape((11, 6)) * un.uV
        ...
        signal3 = AnalogSignal(data3,
                                    units="uV", sampling_rate=1*un.kHz,
                                    name='signal3',
                                    description='test signal',
                                    file_origin='testfile.txt')
        ...
        self.assertEqual(merged13[0, 4], 4*un.mV)
        self.assertEqual(merged23[0, 4], 4*un.mV)
        self.assertEqual(merged13[0, 5], 1*un.mV)
        self.assertEqual(merged23[0, 5], 1*un.mV)
        self.assertEqual(merged13[10, 10], 1.065*un.mV)
        self.assertEqual(merged23[10, 10], 1.065*un.mV)
