...
class Test__generate_datasets(unittest.TestCase):
    ...
    def test__fake_neo__cascade(self):
        ...
        self.assertEqual(res.irregularlysampledsignals[0].annotations, self.annotations)
...
class TestSegment(unittest.TestCase):
    ...
    def check_creation(self, seg):
        ...
        targ4 = get_fake_value("description", str, seed=seed + 4, obj=Segment)
    ...
    def test_times(self):
        ...
        proxy_anasig = AnalogSignalProxy(
            rawio=reader, global_channel_indexes=None, block_index=0, seg_index=0
        )
    ...
    def test__merge(self):
        ...
        assert_same_sub_schema(self.sigarrs1a + self.sigarrs2, seg1a.analogsignals)
    ...
    def test__children(self):
        ...
        totchildren = (
            self.nchildren * 2
            + self.nchildren  # epoch/event
            + self.nchildren ** 2  # analogsignal
            + self.nchildren  # spiketrain
    ...
    def test__size(self):
        targ1 = {
            "epochs": self.nchildren,
            "events": self.nchildren,
    ...
    def test__filter_multi(self):
        ...
        res2 = self.targobj.filter(targdict={"name": self.epcs1a[1].name, "j": 0})
    ...
    def test__filter_multi_nores(self):
        ...
        res8 = self.targobj.filter(targdict={"name": self.epcs2[1].name, "j": 5})
        ...
        res13 = self.targobj.filter(targdict={"name": self.epcs1a[1].name}, j=5)
        res14 = self.targobj.filter(name=self.epcs1a[1].name, targdict={"j": 5})
    ...
    def test__filter_multi_partres(self):
        ...
        res2 = self.targobj.filter(targdict={"name": self.epcs1a[1].name, "j": 5})
    ...
    def test__filter_single_annotation_obj_single(self):
        ...
        res4 = self.targobj.filter(j=1, objects=[Epoch, ChannelIndex])
    ...
    def test__filter_single_attribute_norecur(self):
        targ = [self.epcs1a[1]]
        res0 = self.targobj.filter(name=self.epcs1a[1].name, recursive=False)
        assert_same_sub_schema(res0, targ)

    def test__filter_single_annotation_nodata(self):
        targ = []
        res0 = self.targobj.filter(j=0, data=False)
        assert_same_sub_schema(res0, targ)
    ...
    def test__filter_single_annotation_nodata_norecur(self):
        targ = []
        res0 = self.targobj.filter(j=0, data=False, recursive=False)
        assert_same_sub_schema(res0, targ)

    def test__filter_single_attribute_nodata_norecur(self):
        targ = []
        res0 = self.targobj.filter(name=self.epcs1a[1].name, data=False, recursive=False)
        assert_same_sub_schema(res0, targ)
    ...
    def test__filter_single_attribute_container(self):
        targ = [self.epcs1a[1]]
        res0 = self.targobj.filter(name=self.epcs1a[1].name, container=True)
        assert_same_sub_schema(res0, targ)
    ...
    def test__filter_single_attribute_container_norecur(self):
        targ = [self.epcs1a[1]]
        res0 = self.targobj.filter(name=self.epcs1a[1].name, container=True, recursive=False)
        assert_same_sub_schema(res0, targ)

    def test__filter_single_annotation_nodata_container(self):
        targ = []
        res0 = self.targobj.filter(j=0, data=False, container=True)
        assert_same_sub_schema(res0, targ)

    def test__filter_single_attribute_nodata_container(self):
        targ = []
        res0 = self.targobj.filter(name=self.epcs1a[1].name, data=False, container=True)
        assert_same_sub_schema(res0, targ)

    def test__filter_single_annotation_nodata_container_norecur(self):
        targ = []
        res0 = self.targobj.filter(j=0, data=False, container=True, recursive=False)
        assert_same_sub_schema(res0, targ)

    def test__filter_single_attribute_nodata_container_norecur(self):
        targ = []
        res0 = self.targobj.filter(
            name=self.epcs1a[1].name, data=False, container=True, recursive=False
        )
        assert_same_sub_schema(res0, targ)
    ...
    def test__construct_subsegment_by_unit(self):
        ...
        # channelindexes
        chxs = [
            ChannelIndex(name="Vm", index=unit_with_sig),
            ChannelIndex(name="Conductance", index=unit_with_sig),
        ]
        ...
        for s in range(nb_seg):
            ...
            for j in range(nb_unit):
                st = SpikeTrain([1, 2], units="ms", t_start=0.0, t_stop=10)
                st.unit = all_unit[j]

            for t in signal_types:
                anasigarr = AnalogSignal(
                    np.zeros((sig_len, len(unit_with_sig))),
                    units="nA",
    ...
    def test__time_slice(self):
        time_slice = [0.5, 5.6] * pq.s

        epoch2 = Epoch(
            [0.6, 9.5, 16.8, 34.1] * pq.s,
        ...
        anasig = AnalogSignal(
            np.arange(50.0) * pq.mV, t_start=0.1 * pq.s, sampling_rate=1.0 * pq.Hz
        )
        irrsig = IrregularlySampledSignal(
            signal=np.arange(50.0) * pq.mV, times=anasig.times, t_start=0.1 * pq.s
        )
        st = SpikeTrain(
        ...
        assert_same_attributes(
            sliced.spiketrains[0], st.time_slice(t_start=time_slice[0], t_stop=time_slice[1])
        )
        assert_same_attributes(
            sliced.analogsignals[0], anasig.time_slice(t_start=time_slice[0], t_stop=time_slice[1])
        )
        assert_same_attributes(
        ...
        assert_same_attributes(
            sliced.events[0], event.time_slice(t_start=time_slice[0], t_stop=time_slice[1])
        )
        assert_same_attributes(
            sliced.epochs[0], epoch2.time_slice(t_start=time_slice[0], t_stop=time_slice[1])
        )
        ...
        assert_same_attributes(
            sliced.epochs[0],
            epoch2.time_shift(-time_slice[0]).time_slice(
                t_start=0 * pq.s, t_stop=time_slice[1] - time_slice[0]
            ),
        )
        ...
        sliced_event = loaded_event.time_slice(t_start=time_slice[0], t_stop=time_slice[1])
        has_event = len(sliced_event) > 0

        sliced_anasig = loaded_anasig.time_slice(t_start=time_slice[0], t_stop=time_slice[1])

        sliced_st = loaded_st.time_slice(t_start=time_slice[0], t_stop=time_slice[1])
