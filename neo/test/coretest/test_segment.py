...
from neo.core import AnalogSignal, Block, Event, IrregularlySampledSignal, Epoch, SpikeTrain
from neo.core.spiketrainlist import SpikeTrainList
from neo.core.container import filterdata
from neo.test.tools import assert_neo_object_is_compliant, assert_same_sub_schema, assert_same_attributes
...
class TestSegment(unittest.TestCase):
    ...
    def test_times(self):
        for seg in self.segments:
            ...
            t_starts, t_stops = [], []
            for children in [seg.analogsignals, seg.epochs, seg.events, seg.irregularlysampledsignals, seg.spiketrains]:
        ...
        proxy_anasig = AnalogSignalProxy(
            rawio=reader, stream_index=0, inner_stream_channels=None, block_index=0, seg_index=0
        )
    ...
    def test__merge(self):
        ...
        assert_same_sub_schema(orig_seg1.analogsignals + seg2.analogsignals, seg1.analogsignals)
        assert_same_sub_schema(
            orig_seg1.irregularlysampledsignals + seg2.irregularlysampledsignals, seg1.irregularlysampledsignals
        )
    ...
    def test__filter_single_annotation_obj_multi(self):
        ...
        res0 = segment.filter(thing="wotsit", objects=["Event", SpikeTrain, "AnalogSignal"])
    ...
    def test__time_slice(self):
        ...
        assert_same_attributes(sliced.spiketrains[0], st.time_slice(t_start=time_slice[0], t_stop=time_slice[1]))
        assert_same_attributes(sliced.analogsignals[0], anasig.time_slice(t_start=time_slice[0], t_stop=time_slice[1]))
        assert_same_attributes(
            sliced.irregularlysampledsignals[0], irrsig.time_slice(t_start=time_slice[0], t_stop=time_slice[1])
        )
        assert_same_attributes(sliced.events[0], event.time_slice(t_start=time_slice[0], t_stop=time_slice[1]))
        assert_same_attributes(sliced.epochs[0], epoch2.time_slice(t_start=time_slice[0], t_stop=time_slice[1]))
        ...
        assert_same_attributes(
            ...
            st.time_shift(-time_slice[0]).time_slice(t_start=0 * pq.s, t_stop=time_slice[1] - time_slice[0]),
        )
        ...
        assert_same_attributes(
            sliced.events[0],
            event.time_shift(-time_slice[0]).time_slice(t_start=0 * pq.s, t_stop=time_slice[1] - time_slice[0]),
        )
        assert_same_attributes(
            sliced.epochs[0],
            epoch2.time_shift(-time_slice[0]).time_slice(t_start=0 * pq.s, t_stop=time_slice[1] - time_slice[0]),
        )
    ...
    def test__deepcopy(self):
        childconts = ("analogsignals", "epochs", "events", "irregularlysampledsignals", "spiketrains")
        for segment in self.segments:
