...
class TestEpoch(unittest.TestCase):
    ...
    def test_Epoch_creation(self):
        ...
        assert_arrays_equal(
            epc.labels, np.array(["test epoch 1", "test epoch 2", "test epoch 3"], dtype="U")
        )
    ...
    def test_Epoch_creation_invalid_durations_labels(self):
        self.assertRaises(
            ValueError, Epoch, [1.1, 1.5, 1.7] * pq.ms, durations=[20, 40, 60, 80] * pq.ns
        )
        self.assertRaises(
            ValueError,
            Epoch,
    ...
    def test__time_slice_deepcopy_array_annotations(self):
        ...
        self.assertFalse(
            all(self.epc.array_annotations["test0"][1:4] == result.array_annotations["test0"])
        )
        self.assertFalse(
            all(self.epc.array_annotations["test1"][1:4] == result.array_annotations["test1"])
        )
        self.assertFalse(
            all(self.epc.array_annotations["test2"][1:4] == result.array_annotations["test2"])
        )
    ...
    def test_slice(self):
        ...
        epc = Epoch(
            times * pq.ms, durations=durations * pq.ms, labels=labels, array_annotations=arr_ann
        )
    ...
    def test_rescale(self):
        ...
        assert_arrays_almost_equal(
            result.times.magnitude, np.array([2000, 3000, 4000, 5000]), 1e-9
        )
        assert_arrays_almost_equal(
            result.durations.magnitude, np.array([100, 200, 300, 400]), 1e-9
        )
