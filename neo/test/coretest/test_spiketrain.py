...
class Test__generate_datasets(unittest.TestCase):
    ...
    def test__get_fake_values(self):
        ...
        t_start = get_fake_value('t_start', Units.Quantity, seed=1, dim=0)
        t_stop = get_fake_value('t_stop', Units.Quantity, seed=2, dim=0)
        waveforms = get_fake_value('waveforms', Units.Quantity, seed=3, dim=3)
        left_sweep = get_fake_value('left_sweep', Units.Quantity, seed=4, dim=0)
        sampling_rate = get_fake_value('sampling_rate', Units.Quantity,
                                       seed=5, dim=0)
...
class Testcheck_has_dimensions_time(unittest.TestCase):
    def test__check_has_dimensions_time(self):
        a = np.arange(3) * Units.ms
        b = np.arange(3) * Units.mV
        c = np.arange(3) * Units.mA
        d = np.arange(3) * Units.minute
...
class Testcheck_time_in_range(unittest.TestCase):
    ...
    def test__check_time_in_range_scale(self):
        value = np.array([0., 5000., 10000.])*Units.ms
        t_start = 0.*Units.s
        t_stop = 10.*Units.s
        _check_time_in_range(value, t_start=t_start, t_stop=t_stop)
        _check_time_in_range(value, t_start=t_start, t_stop=t_stop, view=False)
    ...
    def test__check_time_in_range_inside(self):
        value = np.array([0.1, 5., 9.9])*Units.s
        t_start = 0.*Units.s
        t_stop = 10.*Units.s
        _check_time_in_range(value, t_start=t_start, t_stop=t_stop)
        _check_time_in_range(value, t_start=t_start, t_stop=t_stop, view=False)
    ...
    def test__check_time_in_range_below(self):
        value = np.array([-0.1, 5., 10.])*Units.s
        t_start = 0.*Units.s
        t_stop = 10.*Units.s
    ...
    def test__check_time_in_range_below_scale(self):
        value = np.array([-1., 5000., 10000.])*Units.ms
        t_start = 0.*Units.s
        t_stop = 10.*Units.s
        self.assertRaises(ValueError, _check_time_in_range, value,
                          t_start=t_start, t_stop=t_stop)
    ...
    def test__check_time_in_range_above(self):
        value = np.array([0., 5., 10.1])*Units.s
        t_start = 0.*Units.s
        t_stop = 10.*Units.s
        self.assertRaises(ValueError, _check_time_in_range, value,
                          t_start=t_start, t_stop=t_stop)
    ...
    def test__check_time_in_range_above_scale(self):
        value = np.array([0., 5000., 10001.])*Units.ms
        t_start = 0.*Units.s
        t_stop = 10.*Units.s
        self.assertRaises(ValueError, _check_time_in_range, value,
                          t_start=t_start, t_stop=t_stop)
    ...
    def test__check_time_in_range_above_below(self):
        value = np.array([-0.1, 5., 10.1])*Units.s
        t_start = 0.*Units.s
        t_stop = 10.*Units.s
        self.assertRaises(ValueError, _check_time_in_range, value,
                          t_start=t_start, t_stop=t_stop)
    ...
    def test__check_time_in_range_above_below_scale(self):
        value = np.array([-1., 5000., 10001.])*Units.ms
        t_start = 0.*Units.s
        t_stop = 10.*Units.s
        self.assertRaises(ValueError, _check_time_in_range, value,
                          t_start=t_start, t_stop=t_stop)
...
class TestConstructor(unittest.TestCase):
    ...
    def test__create_minimal(self):
        t_start = 0.0
        t_stop = 10.0
        train1 = SpikeTrain([]*Units.s, t_stop)
        train2 = _new_spiketrain(SpikeTrain, []*Units.s, t_stop)
    ...
    def test__create_from_list_set_dtype(self):
        ...
        st_out = Units.Quantity(times, units=units, dtype=dtype)
        self.result_spike_check(train1, st_out, t_start_out, t_stop_out,
                                dtype, units)
        self.result_spike_check(train2, st_out, t_start_out, t_stop_out,
                                dtype, units)
    ...
    def test__create_from_list_no_start_stop_units_set_dtype(self):
        ...
        dtype = np.float32
        units = 1 * Units.ms
        t_start_out = Units.Quantity(t_start, units=units, dtype=dtype)
        t_stop_out = Units.Quantity(t_stop, units=units, dtype=dtype)
        st_out = Units.Quantity(times, units=units, dtype=dtype)
        self.result_spike_check(train1, st_out, t_start_out, t_stop_out,
                                dtype, units)
        self.result_spike_check(train2, st_out, t_start_out, t_stop_out,
    ...
    def test__create_from_quantity_array(self):
        times = np.arange(10) * Units.ms
        t_start = 0.0*Units.s
        t_stop = 12.0*Units.ms
        train1 = SpikeTrain(times, t_start=t_start, t_stop=t_stop)
        train2 = _new_spiketrain(SpikeTrain, times,
                                 t_start=t_start, t_stop=t_stop)
    ...
    def test__create_from_quantity_array_with_dtype(self):
        times = np.arange(10, dtype='f4') * Units.ms
        t_start = 0.0*Units.s
        t_stop = 12.0*Units.ms
        train1 = SpikeTrain(times, t_start=t_start, t_stop=t_stop)
        train2 = _new_spiketrain(SpikeTrain, times,
    ...
    def test_defaults(self):
        ...
        self.assertEqual(train1.sampling_rate, 1.0 * Units.Hz)
        self.assertEqual(train2.sampling_rate, 1.0 * Units.Hz)
    ...
    def test_default_tstart(self):
        ...
        train11 = SpikeTrain([3, 4, 5]*Units.s, t_stop=8000*Units.ms)
        train21 = _new_spiketrain(SpikeTrain, [3, 4, 5]*Units.s,
                                  t_stop=8000*Units.ms)
        assert_neo_object_is_compliant(train11)
        assert_neo_object_is_compliant(train21)
        self.assertEqual(train11.t_start, 0.*Units.s)
        self.assertEqual(train21.t_start, 0.*Units.s)

        # unless otherwise specified
        train12 = SpikeTrain([3, 4, 5]*Units.s, t_start=2.0, t_stop=8)
        train22 = _new_spiketrain(SpikeTrain, [3, 4, 5]*Units.s,
                                  t_start=2.0, t_stop=8)
        assert_neo_object_is_compliant(train12)
        assert_neo_object_is_compliant(train22)
        self.assertEqual(train12.t_start, 2.*Units.s)
        self.assertEqual(train22.t_start, 2.*Units.s)

    def test_tstop_units_conversion(self):
        train11 = SpikeTrain([3, 5, 4]*Units.s, t_stop=10)
        train21 = _new_spiketrain(SpikeTrain, [3, 5, 4]*Units.s, t_stop=10)
        assert_neo_object_is_compliant(train11)
        assert_neo_object_is_compliant(train21)
        self.assertEqual(train11.t_stop, 10.*Units.s)
        self.assertEqual(train21.t_stop, 10.*Units.s)

        train12 = SpikeTrain([3, 5, 4]*Units.s, t_stop=10000.*Units.ms)
        train22 = _new_spiketrain(SpikeTrain, [3, 5, 4]*Units.s,
                                  t_stop=10000.*Units.ms)
        assert_neo_object_is_compliant(train12)
        assert_neo_object_is_compliant(train22)
        self.assertEqual(train12.t_stop, 10.*Units.s)
        self.assertEqual(train22.t_stop, 10.*Units.s)

        train13 = SpikeTrain([3, 5, 4], units='sec', t_stop=10000.*Units.ms)
        train23 = _new_spiketrain(SpikeTrain, [3, 5, 4],
                                  units='sec', t_stop=10000.*Units.ms)
        assert_neo_object_is_compliant(train13)
        assert_neo_object_is_compliant(train23)
        self.assertEqual(train13.t_stop, 10.*Units.s)
        self.assertEqual(train23.t_stop, 10.*Units.s)


class TestSorting(unittest.TestCase):
    def test_sort(self):
        waveforms = np.array([[[0., 1.]], [[2., 3.]], [[4., 5.]]]) * Units.mV
        train = SpikeTrain([3, 4, 5]*Units.s, waveforms=waveforms, name='n',
                           t_stop=10.0)
        assert_neo_object_is_compliant(train)
        train.sort()
        assert_neo_object_is_compliant(train)
        assert_arrays_equal(train, [3, 4, 5]*Units.s)
        assert_arrays_equal(train.waveforms, waveforms)
        self.assertEqual(train.name, 'n')
        self.assertEqual(train.t_stop, 10.0 * Units.s)

        train = SpikeTrain([3, 5, 4]*Units.s, waveforms=waveforms, name='n',
                           t_stop=10.0)
        assert_neo_object_is_compliant(train)
        train.sort()
        assert_neo_object_is_compliant(train)
        ...
        self.assertEqual(train.name, 'n')
        self.assertEqual(train.t_start, 0.0 * Units.s)
        self.assertEqual(train.t_stop, 10.0 * Units.s)
...
class TestSlice(unittest.TestCase):
    def setUp(self):
        ...
        self.data1quant = self.data1*Units.s
...
class TestTimeSlice(unittest.TestCase):
    def setUp(self):
        ...
        self.data1quant = self.data1*Units.ms
        self.train1 = SpikeTrain(self.data1quant, t_stop=10.0*Units.ms,
    ...
    def test_time_slice_typical(self):
        ...
        t_start = 0.12 * Units.ms
        t_stop = 3.5 * Units.ms
        result = self.train1.time_slice(t_start, t_stop)
        targ = SpikeTrain([0.5, 1.2, 3.3] * Units.ms, t_stop=3.3)
    ...
    def test_time_slice_differnt_units(self):
        # time_slice spike train, keep sliced spike times
        t_start = 0.00012 * Units.s
        t_stop = 0.0035 * Units.s
        result = self.train1.time_slice(t_start, t_stop)
        targ = SpikeTrain([0.5, 1.2, 3.3] * Units.ms, t_stop=3.3)
        assert_arrays_equal(result, targ)
        targwaveforms = np.array([[[2., 3.],
    ...
    def test_time_slice_out_of_boundries(self):
        self.train1.t_start = 0.1*Units.ms
        assert_neo_object_is_compliant(self.train1)

        # time_slice spike train, keep sliced spike times
        t_start = 0.01 * Units.ms
        t_stop = 70.0 * Units.ms
        result = self.train1.time_slice(t_start, t_stop)
        assert_arrays_equal(self.train1, result)
        assert_arrays_equal(self.waveforms1, result.waveforms)
    ...
    def test_time_slice_empty(self):
        waveforms = np.array([[[]]]) * Units.mV
        train = SpikeTrain([] * Units.ms, t_stop=10.0, waveforms=waveforms)
        assert_neo_object_is_compliant(train)
    ...
    def test_time_slice_none_stop(self):
        ...
        assert_arrays_equal([1.2, 3.3, 6.4, 7] * Units.ms, result)
    ...
    def test_time_slice_none_start(self):
        ...
        assert_arrays_equal([0.1, 0.5] * Units.ms, result)
        targwaveforms = np.array([[[0., 1.],
                                   [0.1, 1.1]],
                                  [[2., 3.],
                                   [2.1, 3.1]]]) * Units.mV
...
class TestDuplicateWithNewData(unittest.TestCase):
    def setUp(self):
        ...
        self.train = SpikeTrain(self.dataquant, t_stop=10.0*Units.ms,
                                waveforms=self.waveforms)

    def test_duplicate_with_new_data(self):
        signal1 = self.train
        new_t_start = -10*Units.s
        new_t_stop = 10*Units.s
        new_data = np.sort(np.random.uniform(new_t_start.magnitude,
                                             new_t_stop.magnitude,
                                             len(self.train))) * Units.ms
...
class TestAttributesAnnotations(unittest.TestCase):
    ...
    def test_autoset_universally_recommended_attributes(self):
        train = SpikeTrain([3, 4, 5]*Units.s, t_stop=10.0)
        assert_neo_object_is_compliant(train)
    ...
    def test_annotations(self):
        train = SpikeTrain([3, 4, 5]*Units.s, t_stop=11.1)
        assert_neo_object_is_compliant(train)
        self.assertEqual(train.annotations, {})

        train = SpikeTrain([3, 4, 5]*Units.s, t_stop=11.1, ratname='Phillippe')
        assert_neo_object_is_compliant(train)
...
class TestChanging(unittest.TestCase):
    def test_change_with_copy_default(self):
        ...
        train[0] = 99 * Units.s
        assert_neo_object_is_compliant(train)
        self.assertEqual(train[0], 99*Units.s)
        self.assertEqual(data[0], 3*Units.s)
    ...
    def test_change_with_copy_false(self):
        ...
        assert_neo_object_is_compliant(train)
        self.assertEqual(train[0], 99*Units.s)
        self.assertEqual(data[0], 99*Units.s)

    def test_change_with_copy_false_and_fake_rescale(self):
        # Changing spike train also changes data, because it is a view
        # Data source is quantity
        data = [3000, 4000, 5000] * Units.ms
        # even though we specify units, it still returns a view
        train = SpikeTrain(data, units='ms', copy=False, t_stop=100000)
        train[0] = 99000 * Units.ms
        assert_neo_object_is_compliant(train)
        self.assertEqual(train[0], 99000*Units.ms)
        self.assertEqual(data[0], 99000*Units.ms)
    ...
    def test_init_with_rescale(self):
        ...
        assert_neo_object_is_compliant(train)
        self.assertEqual(train[0], 3000*Units.ms)
        self.assertEqual(train._dimensionality, Units.ms._dimensionality)
        self.assertEqual(train.t_stop, 6000*Units.ms)
    ...
    def test_changing_slice_changes_original_spiketrain(self):
        ...
        result[0] = 99 * Units.s
        assert_neo_object_is_compliant(train)
        self.assertEqual(train[1], 99*Units.s)
        self.assertEqual(result[0], 99*Units.s)
        self.assertEqual(data[1], 4*Units.s)
    ...
    def test_changing_slice_changes_original_spiketrain_with_copy_false(self):
        ...
        self.assertEqual(train[1], 99*Units.s)
        self.assertEqual(result[0], 99*Units.s)
        self.assertEqual(data[1], 99*Units.s)

    def test__changing_spiketime_should_check_time_in_range(self):
        data = [3, 4, 5] * Units.ms
        train = SpikeTrain(data, copy=False, t_start=0.5, t_stop=10.0)
        assert_neo_object_is_compliant(train)
        self.assertRaises(ValueError, train.__setitem__, 0, 10.1*Units.ms)
        self.assertRaises(ValueError, train.__setitem__, 1, 5.0*Units.s)
        self.assertRaises(ValueError, train.__setitem__, 2, 5.0*Units.s)
        self.assertRaises(ValueError, train.__setitem__, 0, 0)

    def test__changing_multiple_spiketimes(self):
        data = [3, 4, 5] * Units.ms
        train = SpikeTrain(data, copy=False, t_start=0.5, t_stop=10.0)
        train[:] = [7, 8, 9] * Units.ms
        assert_neo_object_is_compliant(train)
    ...
    def test__rescale(self):
        ...
        result = train.rescale(Units.s)
        assert_neo_object_is_compliant(train)
        assert_neo_object_is_compliant(result)
        assert_arrays_equal(train, result)
        self.assertEqual(result.units, 1 * Units.s)

    def test__rescale_same_units(self):
        data = [3, 4, 5] * Units.ms
        train = SpikeTrain(data, t_start=0.5, t_stop=10.0)
        result = train.rescale(Units.ms)
        assert_neo_object_is_compliant(train)
        assert_arrays_equal(train, result)
        self.assertEqual(result.units, 1 * Units.ms)

    def test__rescale_incompatible_units_ValueError(self):
        data = [3, 4, 5] * Units.ms
        train = SpikeTrain(data, t_start=0.5, t_stop=10.0)
        assert_neo_object_is_compliant(train)
        self.assertRaises(ValueError, train.rescale, Units.m)
...
class TestPropertiesMethods(unittest.TestCase):
    def setUp(self):
        ...
        self.t_start1 = 0.5
        self.t_stop1 = 10.0
        self.t_start1quant = self.t_start1 * Units.ms
        self.t_stop1quant = self.t_stop1 * Units.ms
        self.sampling_rate1 = .1*Units.Hz
        self.left_sweep1 = 2.*Units.s
        self.name1 = 'train 1'
    ...
    def test__duration(self):
        ...
        self.assertEqual(result1, 9.5 * Units.ms)
        self.assertEqual(result1.units, 1. * Units.ms)
    ...
    def test__spike_duration(self):
        ...
        self.assertEqual(result1, 20./Units.Hz)
        self.assertEqual(result1.units, 1./Units.Hz)
        self.assertEqual(result2, None)
        self.assertEqual(result3, None)
    ...
    def test__sampling_period(self):
        ...
        self.train1.sampling_rate = self.sampling_rate1
        self.train1.sampling_period = 10.*Units.ms
        assert_neo_object_is_compliant(self.train1)
        ...
        self.assertEqual(result1, 10./Units.Hz)
        self.assertEqual(result1.units, 1./Units.Hz)
        self.assertEqual(result2, None)
        self.assertEqual(result3a, 10.*Units.ms)
        self.assertEqual(result3a.units, 1.*Units.ms)
        self.assertEqual(result3b, .1/Units.ms)
        self.assertEqual(result3b.units, 1./Units.ms)
    ...
    def test__right_sweep(self):
        ...
        self.assertEqual(result1, 22.*Units.s)
        self.assertEqual(result1.units, 1.*Units.s)
        self.assertEqual(result2, None)
        self.assertEqual(result3, None)
...
class TestMiscellaneous(unittest.TestCase):
    def test__different_dtype_for_t_start_and_array(self):
        data = np.array([0, 9.9999999], dtype=np.float64) * Units.s
