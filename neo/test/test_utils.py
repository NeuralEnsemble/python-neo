...
class TestUtilsWithoutProxyObjects(unittest.TestCase):
    def test__get_events(self):
        starts_1 = Event(
            times=[0.5, 10.0, 25.2] * pq.s, labels=["label1", "label2", "label3"], name="pick_me"
        )
        ...
        start_idx = np.where(
            np.array([ev.annotations["event_type"] for ev in trial_2b]) == "trial start"
        )[0][0]
        ...
        self.assertEqual(
            stops_1.annotations["event_type"], trial_2b_stop.annotations["event_type"]
        )
        ...
        start_idx = np.where(
            np.array([ev.annotations["event_type"] for ev in trials_1_2b]) == "trial start"
        )[0][0]
        ...
        self.assertEqual(
            stops_1.annotations["event_type"], trials_1_2b_stop.annotations["event_type"]
        )
    ...
    def test__get_epochs(self):
        ...
        a_idx = np.where(np.array([ev.annotations["epoch_type"] for ev in trials_1_2b]) == "a")[0][
            0
        ]
    ...
    def test__add_epoch(self):
        ...
        assert_arrays_almost_equal(
            ep_starts.durations,
            (550 * pq.ms).rescale(ep_starts.durations.units) * np.ones(len(starts)),
            1e-12,
        )
    ...
    def test__cut_block_by_epochs(self):
        epoch = Epoch(
            [0.5, 10.0, 25.2] * pq.s, durations=[5.1, 4.8, 5.0] * pq.s, t_start=0.1 * pq.s
        )
        ...
        for epoch_idx in range(len(epoch)):
            ...
            assert_same_attributes(
                block.segments[epoch_idx].irregularlysampledsignals[0], irrsig_target
            )
            assert_same_attributes(
                block.segments[epoch_idx].events[0],
...
class TestUtilsWithProxyObjects(BaseProxyTest):
    def test__get_events(self):
        ...
        proxy_event = EventProxy(
            rawio=self.reader, event_channel_index=0, block_index=0, seg_index=0
        )
    ...
    def test__get_epochs(self):
        ...
        proxy_epoch = EpochProxy(
            rawio=self.reader, event_channel_index=1, block_index=0, seg_index=0
        )
    ...
    def test__add_epoch(self):
        ...
        assert_arrays_almost_equal(
            epoch.durations, np.ones(regular_event.shape) * loaded_event.units, 1e-12
        )
    ...
    def test__cut_block_by_epochs(self):
        ...
        proxy_anasig = AnalogSignalProxy(
            rawio=self.reader, global_channel_indexes=None, block_index=0, seg_index=0
        )
        seg.analogsignals.append(proxy_anasig)

        proxy_st = SpikeTrainProxy(rawio=self.reader, unit_index=0, block_index=0, seg_index=0)
