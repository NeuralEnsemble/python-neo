...
def read_analogsignals(reader):
    ...
    for stream_index in range(nb_stream):
        ...
        for raw_chunk in iter_over_sig_chunks(reader, stream_index, channel_indexes, chunksize=1024):
        ...
        if nb_chan >= 3:
            for prefer_slice in (True, False):
                raw_chunk3 = reader.get_analogsignal_chunk(
                    block_index=block_index,
                    seg_index=seg_index,
                    i_start=i_start,
                    i_stop=i_stop,
        ...
        for dt in ("float32", "float64"):
            float_chunk0 = reader.rescale_signal_raw_to_float(
                raw_chunk0, dtype=dt, stream_index=stream_index, channel_indexes=channel_indexes2
            )
            float_chunk2 = reader.rescale_signal_raw_to_float(
                raw_chunk2, dtype=dt, stream_index=stream_index, channel_ids=channel_ids2
            )
            if unique_chan_name:
                float_chunk1 = reader.rescale_signal_raw_to_float(
                    raw_chunk1, dtype=dt, stream_index=stream_index, channel_names=channel_names2
                )
    ...
    for block_index in range(nb_block):
        ...
        for seg_index in range(nb_seg):
            for ev_chan in range(nb_event_channel):
                nb_event = reader.event_count(block_index=block_index, seg_index=seg_index, event_channel_index=ev_chan)
                if nb_event == 0:
                    continue
