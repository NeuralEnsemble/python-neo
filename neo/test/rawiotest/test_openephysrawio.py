...
class TestOpenEphysRawIO(
    ...
    def test_raise_error_if_discontinuous_files(self):
        ...
        reader = OpenEphysRawIO(dirname=self.get_local_path("openephys/OpenEphys_SampleData_2_(multiple_starts)"))
        with self.assertRaises(ValueError):
            reader.parse_header()
        # if ignore_timestamps_errors=True, no exception is raised
        reader = OpenEphysRawIO(
            dirname=self.get_local_path("openephys/OpenEphys_SampleData_2_(multiple_starts)"),
