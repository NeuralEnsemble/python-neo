...
class CommonTests(BaseTestIO, unittest.TestCase):
    ...
    files_to_test = ["FileSpec2.3001", "blackrock_2_1/l101210-001"]
    ...
    def test_inputs_V21(self):
        ...
        block = reader.read_block(
            load_waveforms=True, signal_group_mode="split-all",
    ...
    def test_compare_blackrockio_with_matlabloader_v21(self):
        ...
        dirname = get_test_file_full_path(
            ioclass=BlackrockIO,
        ...
        for index, param in enumerate(parameters):
            ...
            for st_i in block.segments[0].spiketrains:
                ...
                # Compare waveforms
                matlab_wf = wf_ml[
                    np.nonzero(np.logical_and(elec_ml == channelid, unit_ml == unitid)), :
                ][0]
                ...
                if (
                    not st_i.units == pq.CompoundUnit("1.0/{} * s".format(30000))
                ) and st_i.units == pq.s:
                ...
                if ea_i.name == "digital_input_port":
                    ...
                    for marker_id in marker_ids:
                        python_digievents = np.round(
                            ea_i.times.base[ea_i.labels == marker_id] * 30000
                        ).astype(int)
    ...
    def test_segment_detection_reset(self):
        ...
        self.assertEqual(
            len(block.segments[0].analogsignals), len(block.segments[1].analogsignals)
        )
    ...
    def test_segment_detection_pause(self):
        ...
        with warnings.catch_warnings(record=True) as w:
            ...
            reader = BlackrockIO(
                filename=filename, nsx_to_load=2, nev_override=filename_nev_outside_seg
            )
        ...
        self.assertAlmostEqual(
            block.segments[1].analogsignals[0].t_stop.magnitude, 35.0087, places=6
        )
        self.assertEqual(block.segments[0].spiketrains[0].t_start, 0.0)
        self.assertAlmostEqual(
            block.segments[0].spiketrains[0].t_stop.magnitude, 15.83916667, places=8
        )
        ...
        self.assertEqual(block.segments[1].spiketrains[0].t_start, 31.0087)
        self.assertAlmostEqual(
            block.segments[1].spiketrains[0].t_stop.magnitude, 35.0087, places=6
        )
