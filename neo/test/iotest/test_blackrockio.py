        ...
        for index, param in enumerate(parameters):
            ...
            for st_i in block.segments[0].spiketrains:
                ...
                matlab_wf = wf_ml[np.nonzero(
                    np.logical_and(elec_ml == channelid, unit_ml == unitid)), :][0].T
                wfs = np.squeeze(st_i.waveforms).magnitude
                if len(wfs.shape) == 1:
                    wfs = wfs[:,None] # expanding to two dimensions in case of single waveform
                assert_equal(wfs, matlab_wf)
