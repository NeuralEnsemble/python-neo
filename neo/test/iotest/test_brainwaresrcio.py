...
FILES_TO_TEST = [
    "block_300ms_4rep_1clust_part_ch1.src",
...
def proc_src_condition_unit_repetition(
    sweep, damaIndex, timeStamp, sweepLen, side, ADperiod, respWin, filename
):
    ...
    if len(sweep):
        ...
        shapes = np.concatenate(
            [res.flatten()[np.newaxis][np.newaxis] for res in sweep["shape"]], axis=0
        )
