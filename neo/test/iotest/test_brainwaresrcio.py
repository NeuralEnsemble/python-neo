...
def proc_src_units(srcfile, filename):
    ...
    all_units = []
    un_unit = Group(
        name="UnassignedSpikes", file_origin=filename, elliptic=[], boundaries=[], timestamp=[], max_valid=[]
    )
    ...
    if len(cluster):
        ...
        for ielliptic, iboundaries in fullclust:
            unit = Group(
                file_origin=filename, boundaries=[iboundaries], elliptic=[ielliptic], timeStamp=[], max_valid=[maxValid]
            )
...
def proc_src_condition(rep, filename, ADperiod, side, block):
    ...
    if len(unassignedSpikes):
        ...
        trains = proc_src_condition_unit(
            spikeunit, sweepLen, side, ADperiod, respWin, damaIndexes, timeStamps, filename
        )
    ...
    for trains in atrains:
        segment = Segment(
            file_origin=filename, feature_type=-1, go_by_closest_unit_center=False, include_unit_bounds=False, **stim
        )
        block.segments.append(segment)
        segment.spiketrains = trains


def proc_src_condition_unit(spikeunit, sweepLen, side, ADperiod, respWin, damaIndexes, timeStamps, filename):
    ...
    for sweep, damaIndex, timeStamp in zip(spikeunit, damaIndexes, timeStamps):
        ...
        train = proc_src_condition_unit_repetition(
            sweep, damaIndex, timeStamp, sweepLen, side, ADperiod, respWin, filename
        )
        trains.append(train)
    return trains


def proc_src_condition_unit_repetition(sweep, damaIndex, timeStamp, sweepLen, side, ADperiod, respWin, filename):
    ...
    if len(sweep):
        ...
        shapes = np.concatenate([res.flatten()[np.newaxis][np.newaxis] for res in sweep["shape"]], axis=0)
...
class BrainwareSrcIOTestCase(BaseTestIO, unittest.TestCase):
    ...
    entities_to_download = ["brainwaresrc"]
    ...
    def test_against_reference(self):
        for filename, refname in zip(self.files_to_test, self.files_to_compare):
            if not refname:
                continue
...
if __name__ == "__main__":
    logger = logging.getLogger(BrainwareSrcIO.__module__ + "." + BrainwareSrcIO.__name__)
    logger.setLevel(100)
    unittest.main()
