...
class TestNestIO_Analogsignals(BaseTestIO, unittest.TestCase):
    ...
    def test_id_column_none_multiple_neurons(self):
        ...
        filename = get_test_file_full_path(
            ioclass=NestIO, filename="0time-1255-0.gdf", directory=self.local_test_dir, clean=False
        )
        r = NestIO(filenames=filename)
    ...
    def test_multiple_value_columns(self):
        ...
        sampling_period = pq.CompoundUnit("5*ms")
        seg = r.read_segment(
            gid_list=[1001], value_columns_dat=[2, 3], sampling_period=sampling_period
        )
        anasigs = seg.analogsignals
...
class TestNestIO_Spiketrains(BaseTestIO, unittest.TestCase):
    ...
    def test_read_spiketrain(self):
        ...
        r = NestIO(filenames=filename)
        r.read_spiketrain(
            t_start=400.0 * pq.ms, t_stop=500.0 * pq.ms, lazy=False, id_column=None, time_column=0
        )
        r.read_segment(
            t_start=400.0 * pq.ms,
            t_stop=500.0 * pq.ms,
    ...
    def test_adding_custom_annotation(self):
        ...
        r = NestIO(filenames=filename)
        st = r.read_spiketrain(
            gdf_id=0, t_start=400.0 * pq.ms, t_stop=500.0 * pq.ms, layer="L23", population="I"
        )
    ...
    def test_t_start_undefined_raises_error(self):
        ...
        with self.assertRaises(ValueError):
            r.read_spiketrain(
                gdf_id=1, t_stop=500.0 * pq.ms, lazy=False, id_column=0, time_column=1
            )
        with self.assertRaises(ValueError):
            r.read_segment(
                gid_list=[1, 2, 3],
    ...
    def test_t_stop_undefined_raises_error(self):
        ...
        with self.assertRaises(ValueError):
            r.read_spiketrain(
                gdf_id=1, t_start=400.0 * pq.ms, lazy=False, id_column=0, time_column=1
            )
        with self.assertRaises(ValueError):
            r.read_segment(
                gid_list=[1, 2, 3],
...
class TestNestIO_multiple_signal_types(BaseTestIO, unittest.TestCase):
    ioclass = NestIO
    files_to_test = []
    files_to_download = ["0gid-1time-2gex-3Vm-1261-0.dat", "0gid-1time_in_steps-1258-0.gdf"]
    ...
    def test_read_analogsignal_and_spiketrain(self):
        ...
        using read_segment
        """
        files = ["0gid-1time-2gex-3Vm-1261-0.dat", "0gid-1time_in_steps-1258-0.gdf"]
        filenames = [
            get_test_file_full_path(
                ioclass=NestIO, filename=file, directory=self.local_test_dir, clean=False
            )
            for file in files
        ]
