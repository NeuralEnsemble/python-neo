...
class BaseTestIO(object):
    ...
    def read_file(self, filename=None, return_path=False, clean=False,
        ...
        If return_path is True, return the full path of the file along with
        the object.  return ob, path.  Default is False.
        ...
        ioobj, path = self.generic_io_object(filename=filename,
                                             return_path=True, clean=clean)
        ob = read_generic(ioobj, target=target, cascade=cascade, lazy=lazy,
    ...
    def write_file(self, ob=None, filename=None, return_path=False,
                   clean=False, target=None):
        '''
        ...
        ioobj, path = self.generic_io_object(filename=filename,
                                             return_path=True, clean=clean)
        ob = write_generic(ioobj, target=target, return_reader=False)
    ...
    def test_write_then_read(self):
        ...
        for cascade in self.cascade_modes:
            ioobj1, path1 = self.generic_io_object(return_path=True,
                                                   clean=True)
            if ioobj1 is None:
                return
            ...
            ioobj2, path = self.generic_io_object(return_path=True)

            # Read the highest supported object from the file
            ob_reader = create_generic_reader(ioobj2, target=False)
            ob2 = ob_reader(cascade=cascade)[0]
    ...
    def test_assert_readed_neo_object_is_compliant(self):
        ...
        for cascade in self.cascade_modes:
            for lazy in [True, False]:
                for ob, path in self.iter_objects(cascade=cascade, lazy=lazy,
                                                  return_path=True):
                    try:
                    ...
                    # intercept exceptions and add more information
                        ...
                        e.args += ('from %s with cascade=%s and lazy=%s' %
    ...
    def test_readed_with_cascade_is_compliant(self):
        ...
        """ % self.ioclass.__name__
        # This is for files presents at G-Node or generated
        for ob, path in self.iter_objects(cascade=False, lazy=False,
                                          return_path=True):
            try:
                # Check compliance of the block or segment
                assert_neo_object_is_compliant(ob)
                assert_children_empty(ob, self.ioclass)
            # intercept exceptions and add more information
            except BaseException as e:
                e.args += ('from %s ' % os.path.basename(path),)
                raise
    ...
    def test_readed_with_lazy_is_compliant(self):
        ...
        # This is for files presents at G-Node or generated
        for cascade in self.cascade_modes:
            for ob, path in self.iter_objects(cascade=cascade, lazy=True,
                                              return_path=True):
                try:
                    assert_sub_schema_is_lazy_loaded(ob)
                # intercept exceptions and add more information
                except BaseException as e:
                    e.args += ('from %s with cascade=%s ' %
    ...
    def test_load_lazy_objects(self):
        ...
        # This is for files presents at G-Node or generated
        for cascade in self.cascade_modes:
            for ob, path, ioobj in self.iter_objects(cascade=cascade,
                                                     lazy=True,
                                                     return_ioobj=True,
                                                     return_path=True):
                try:
                    assert_lazy_sub_schema_can_be_loaded(ob, ioobj)
                # intercept exceptions and add more information
                except BaseException as e:
                    e.args += ('from %s with cascade=%s ' %
                               (os.path.basename(path), cascade),)
