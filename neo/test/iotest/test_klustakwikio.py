...
class testFilenameParser(unittest.TestCase):
    ...
    def setUp(self):
        self.dirname = os.path.join(
            tempfile.gettempdir(), "files_for_testing_neo", "klustakwik/test1"
        )
        if not os.path.exists(self.dirname):
            raise unittest.SkipTest("data directory does not exist: " + self.dirname)
...
class testRead(unittest.TestCase):
    ...
    def setUp(self):
        self.dirname = os.path.join(
            tempfile.gettempdir(), "files_for_testing_neo", "klustakwik/test2"
        )
        if not os.path.exists(self.dirname):
            raise unittest.SkipTest("data directory does not exist: " + self.dirname)
    ...
    def test1(self):
        ...
        kio = KlustaKwikIO(filename=os.path.join(self.dirname, "base"), sampling_rate=1000.0)
        block = kio.read()[0]
        ...
        self.assertTrue(np.all(seg.spiketrains[0].times == np.array([0.100, 0.200])))
        ...
        self.assertTrue(np.all(seg.spiketrains[1].times == np.array([0.305])))
        ...
        self.assertTrue(np.all(seg.spiketrains[2].times == np.array([0.253])))
        ...
        self.assertTrue(np.all(seg.spiketrains[3].times == np.array([0.050, 0.152])))

    def test2(self):
        """Checks that cluster id autosets to 0 without clu file"""
        kio = KlustaKwikIO(filename=os.path.join(self.dirname, "base2"), sampling_rate=1000.0)
        block = kio.read()[0]
        seg = block.segments[0]
        ...
        self.assertTrue(np.all(seg.spiketrains[0].times == np.array([0.026, 0.122, 0.228])))
...
class testWrite(unittest.TestCase):
    def setUp(self):
        self.dirname = os.path.join(
            tempfile.gettempdir(), "files_for_testing_neo", "klustakwik/test3"
        )
        if not os.path.exists(self.dirname):
            raise unittest.SkipTest("data directory does not exist: " + self.dirname)
    ...
    def test1(self):
        ...
        st1 = neo.SpikeTrain(times=[0.002, 0.004, 0.006], units="s", t_stop=1.0)
        ...
        st3 = neo.SpikeTrain(times=[0.05, 0.09, 0.10], units="s", t_stop=1.0)
        ...
        st4 = neo.SpikeTrain(times=[0.005, 0.009], units="s", t_stop=1.0)
        ...
        kio = KlustaKwikIO(filename=os.path.join(self.dirname, "base1"), sampling_rate=1000.0)
        kio.write_block(block)

        # Check files were created
        for fn in [".fet.0", ".fet.1", ".clu.0", ".clu.1"]:
            self.assertTrue(os.path.exists(os.path.join(self.dirname, "base1" + fn)))
...
class testWriteWithFeatures(unittest.TestCase):
    def setUp(self):
        self.dirname = os.path.join(
            tempfile.gettempdir(), "files_for_testing_neo", "klustakwik/test4"
        )
        if not os.path.exists(self.dirname):
            raise unittest.SkipTest("data directory does not exist: " + self.dirname)
    ...
    def test1(self):
        ...
        for fn in [".fet.0", ".clu.0"]:
            self.assertTrue(os.path.exists(os.path.join(self.dirname, "base2" + fn)))
        ...
        assert_arrays_almost_equal(wff, np.array(new_wff), 0.00001)
        ...
        assert_arrays_almost_equal(wff, train.annotations["waveform_features"], 0.00001)
...
def delete_test_session(dirname=None):
    ...
    if dirname is None:
        dirname = os.path.join(os.path.dirname(__file__), "files_for_tests/klustakwik/test3")
