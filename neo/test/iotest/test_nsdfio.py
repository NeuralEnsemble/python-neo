...
class NSDFIOTest(unittest.TestCase):
    ...
    def _create_block_children(self, block):
        ...
        for i in range(3):
            block.channel_indexes.append(
                self.create_channelindex(
                    block,
                    name="ChannelIndex #{}".format(i),
    ...
    def _create_segment_children(self, segment):
        for i in range(2):
            segment.analogsignals.append(
                self.create_analogsignal(segment, name="Signal #{}".format(i * 3))
            )
            segment.analogsignals.append(
                self.create_analogsignal2(segment, name="Signal #{}".format(i * 3 + 1))
            )
            segment.analogsignals.append(
                self.create_analogsignal3(segment, name="Signal #{}".format(i * 3 + 2))
            )

    def create_analogsignal(self, parent=None, name="AnalogSignal1"):
        signal = AnalogSignal(
            [[1.0, 2.5], [2.2, 3.1], [3.2, 4.4]],
            units="mV",
    ...
    def create_analogsignal2(self, parent=None, name="AnalogSignal2"):
        signal = AnalogSignal([[1], [2], [3], [4], [5]], units="mA", sampling_period=0.5 * pq.ms)
    ...
    def create_analogsignal3(self, parent=None, name="AnalogSignal3"):
        signal = AnalogSignal(
            [[1, 2, 3], [4, 5, 6]], units="mV", sampling_rate=2 * pq.kHz, t_start=100 * pq.s
        )
    ...
    def _assign_annotations(self, object):
        object.annotations = {"str": "value", "int": 56, "float": 5.234}
...
class NSDFIOTestWriteThenRead(NSDFIOTest):
    ...
    def _compare_objects(self, object1, object2, exclude_attr=[]):
        ...
        assert_same_attributes(
            object1, object2, exclude=["file_origin", "file_datetime"] + exclude_attr
        )
