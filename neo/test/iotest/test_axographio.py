...
class TestAxographIO(BaseTestIO, unittest.TestCase):
    ...
    def test_version_1(self):
        ...
        arr = sig.as_array('pA')
        target = np.array([[-5.507813 ],
                           [-3.117188 ],
                           [ 1.6640626],
                           [ 1.6640626],
                           [ 4.054688 ]], dtype=np.float32)
    ...
    def test_events_and_epochs(self):
        ...
        assert_equal(event.times, [ 5999,  5999, 23499, 23499, 26499, 26499, 35999] * blk.segments[0].analogsignals[0].sampling_period)
        assert_equal(event.labels, [b'Stop', b'Start', b'Stop', b'Start', b'Stop', b'Start', b'Stop'])
