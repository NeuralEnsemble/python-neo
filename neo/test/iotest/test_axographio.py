...
class TestAxographIO(BaseTestIO, unittest.TestCase):
    ...
    def test_version_1(self):
        ...
        arr = sig.as_array("pA")
        target = np.array(
            [[-5.5078130], [-3.1171880], [+1.6640626], [+1.6640626], [+4.0546880]],
            dtype=np.float32,
        )
    ...
    def test_version_5(self):
        ...
        arr = sig.as_array("pA")
        target = np.array(
            [[+3.0846775], [-2.5403225], [-1.2903225], [+6.8346770], [-5.0403230]],
            dtype=np.float32,
        )
        assert_equal(arr, target)
    ...
    def test_version_6(self):
        ...
        arr = sig.as_array("mV")
        target = np.array(
            [[-60.731834], [-60.701313], [-60.670795], [-60.701313], [-60.731834]],
            dtype=np.float32,
        )
        assert_equal(arr, target)
    ...
    def test_file_written_by_axographio_package_with_linearsequence(self):
        ...
        arr = sig.as_array("mV")
        target = np.array(
            [[0.000000], [9.999833], [19.998667], [29.995500], [39.989334]], dtype=np.float32
        )
        assert_equal(arr, target)
    ...
    def test_multi_segment(self):
        ...
        arr = sig.as_array("V")
        target = np.array(
            [[1.37500e-06], [1.53125e-06], [1.34375e-06], [1.09375e-06], [1.21875e-06]],
            dtype=np.float32,
        )
        assert_equal(arr, target)
    ...
    def test_events_and_epochs(self):
        ...
        assert_equal(
            event.times,
            [5999, 5999, 23499, 23499, 26499, 26499, 35999]
