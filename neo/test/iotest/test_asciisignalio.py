...
class TestAsciiSignalIO(unittest.TestCase):
    def test_genfromtxt_expect_success(self):
        ...
        io = AsciiSignalIO(
            filename, sampling_rate=sampling_rate, delimiter=" ", units="mV", method="genfromtxt"
        )
        block = io.read_block()
    ...
    def test_csv_expect_success(self):
        ...
        assert_array_almost_equal(
            signal[:, 1].reshape(-1).magnitude, np.array(sample_data)[:, 1], decimal=5
        )
    ...
    def test_callable_expect_success(self):
        ...
        def reader(filename, comment_rows):
            return np.genfromtxt(
                filename, delimiter=" ", usecols=None, skip_header=comment_rows or 0, dtype="f"
            )

        io = AsciiSignalIO(
            filename, sampling_rate=sampling_rate, delimiter=" ", units="mV", method=reader
        )
        block = io.read_block()
    ...
    def test_roundtrip_with_json_metadata(self):
        ...
        assert_array_almost_equal(
            signal1.magnitude[:, 1], signal2.magnitude.reshape(-1), decimal=7
        )
