...
class TestAnalogSignalProxy(BaseProxyTest):
    def test_AnalogSignalProxy(self):
        ...
        anasig = proxy_anasig.load(time_slice=(2.0 * pq.s, 5 * pq.s), channel_indexes=[3, 4, 9])
    ...
    def test_global_local_channel_indexes(self):
        proxy_anasig = AnalogSignalProxy(
            rawio=self.reader, global_channel_indexes=slice(0, 10, 2), block_index=0, seg_index=0
        )
...
class TestSpikeTrainProxy(BaseProxyTest):
    def test_SpikeTrainProxy(self):
        proxy_sptr = SpikeTrainProxy(rawio=self.reader, unit_index=0, block_index=0, seg_index=0)
