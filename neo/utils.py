    ...
    # dictionary is given
        ...
        for ep in input_list:
            if isinstance(ep, neo.Epoch) or isinstance(ep, neo.Event):
                sparse_ep = ep.copy()
            elif isinstance(ep, neo.io.proxyobjects.EpochProxy) or isinstance(
                ep, neo.io.proxyobjects.EventProxy
            ):
...
def add_epoch(
    segment, event1, event2=None, pre=0 * pq.s, post=0 * pq.s, attach_result=True, **kwargs
):
    """
    ...
    if any(durations < 0):
        raise ValueError(
            "Can not create epoch with negative duration. " "Requested durations %s." % durations
        )
    ...
    if "labels" not in kwargs:
        kwargs["labels"] = [u"{}_{}".format(kwargs["name"], i) for i in range(len(times))]
...
def cut_segment_by_epoch(seg, epoch, reset_time=False):
    ...
    for ep_id in range(len(epoch)):
        subseg = seg.time_slice(
            epoch.times[ep_id], epoch.times[ep_id] + epoch.durations[ep_id], reset_time=reset_time
        )
        ...
        for i in range(n_sig):
            for seg in block.segments:
                if seg.analogsignals[i].sampling_rate != seg0.analogsignals[i].sampling_rate:
                    problems.append(
                        "AnalogSignals have inconsistent sampling rate across segments"
                    )
                if seg.analogsignals[i].shape[1] != seg0.analogsignals[i].shape[1]:
                    problems.append(
                        "AnalogSignals have inconsistent channel count across segments"
                    )
