...
class BlackrockRawIO(BaseRawIO):
    ...
    extensions.extend(
        ["nev",]
    )  # 'sif', 'ccf' not yet supported
    rawmode = "multi-file"

    def __init__(
        self, filename=None, nsx_override=None, nev_override=None, nsx_to_load=None, verbose=False
    ):
        """
    ...
    def _parse_header(self):
        ...
        assert all(
            all_spec[0] == spec for spec in all_spec
        ), "Files don't have the same internal version"

        if (
            len(self.nsx_to_load) > 0
            and self.__nsx_spec[self.nsx_to_load[0]] == "2.1"
            and not self._avail_files["nev"]
        ):
            pass
            ...
            for nsx_nb in self.nsx_to_load:
                ...
                for i, chan in enumerate(ext_header):
                    ...
                    if np.isnan(float(chan["min_analog_val"])):
                        ...
                        offset = -float(chan["min_digital_val"]) * gain + float(
                            chan["min_analog_val"]
                        )
                    group_id = nsx_nb
                    sig_channels.append(
                        (ch_name, ch_id, sr, sig_dtype, units, gain, offset, group_id,)
                    )
        ...
        for c in range(sig_channels.size):
            ...
            if self._avail_files["nev"]:
                ...
                if sig_channels[c]["id"] in neuevwav["electrode_id"]:
                    ...
                    chidx_ann["nev_energy_threshold"] = (
                        neuevwav["energy_threshold"][get_idx]
                        * chidx_ann["nev_dig_factor"]
                        / 1000
                        * pq.uV
                    )
                    chidx_ann["nev_hi_threshold"] = (
                        neuevwav["hi_threshold"][get_idx]
                        * chidx_ann["nev_dig_factor"]
                        / 1000
                        * pq.uV
                    )
                    chidx_ann["nev_lo_threshold"] = (
                        neuevwav["lo_threshold"][get_idx]
                        * chidx_ann["nev_dig_factor"]
                        / 1000
                        * pq.uV
                    )
                    ...
                    if self.__nev_spec in ["2.2", "2.3"]:
                        ...
                        get_idx = list(neuevflt["electrode_id"]).index(sig_channels[c]["id"])
                        # filter type codes (extracted from blackrock manual)
                        chidx_ann["nev_hi_freq_corner"] = (
                            neuevflt["hi_freq_corner"][get_idx] / 1000.0 * pq.Hz
                        )
                        chidx_ann["nev_hi_freq_order"] = neuevflt["hi_freq_order"][get_idx]
                        chidx_ann["nev_hi_freq_type"] = flt_type[neuevflt["hi_freq_type"][get_idx]]
                        chidx_ann["nev_lo_freq_corner"] = (
                            neuevflt["lo_freq_corner"][get_idx] / 1000.0 * pq.Hz
                        )
            ...
            if self.__nsx_spec[self.nsx_to_load[0]] in ["2.2", "2.3"] and self.__nsx_ext_header:
                ...
                if sig_channels[c]["id"] in self.__nsx_ext_header[k]["electrode_id"]:
                    get_idx = list(self.__nsx_ext_header[k]["electrode_id"]).index(
                        sig_channels[c]["id"]
                    )
                    ...
                    chidx_ann["nsx_hi_freq_corner"] = (
                        self.__nsx_ext_header[k]["hi_freq_corner"][get_idx] / 1000.0 * pq.Hz
                    )
                    chidx_ann["nsx_lo_freq_corner"] = (
                        self.__nsx_ext_header[k]["lo_freq_corner"][get_idx] / 1000.0 * pq.Hz
                    )
                    chidx_ann["nsx_hi_freq_order"] = self.__nsx_ext_header[k]["hi_freq_order"][
                        get_idx
                    ]
                    chidx_ann["nsx_lo_freq_order"] = self.__nsx_ext_header[k]["lo_freq_order"][
                        get_idx
                    ]
        ...
        for seg_index in range(self._nb_segment):
            ...
            for c in range(sig_channels.size):
                ...
                chidx_ann = self.raw_annotations["signal_channels"][c]
                chidx_ann["description"] = (
                    "Container for Units and AnalogSignals of "
    ...
    def _get_event_timestamps(self, block_index, seg_index, event_channel_index, t_start, t_stop):
        ...
        if name == "comments":
            ...
            labels = [
                data[ev_dict["field"]].decode(encoding[data["char_set"]]) for data in events_data
            ]
    ...
    def __match_nsx_and_nev_segment_ids(self, nsx_nb):
        ...
        if self.__nev_spec == "2.3":
            ...
            for k, (data, ev_ids) in self.nev_data.items():
                ...
                for i, seg in enumerate(list_nonempty_nsx_segments[:]):
                    ...
                    if len(data[mask_outside]) > 0:
                        warnings.warn(
                            "Spikes outside any segment. Detected on segment #{}".format(i)
                        )
            ...
            assert nb_possible_nev_segments == len(nonempty_nsx_segments), (
                ...
                "ns{0} file.".format(nsx_nb, nb_possible_nev_segments, len(nonempty_nsx_segments))
            )

            new_nev_segment_id_mapping = dict(
                zip(range(nb_possible_nev_segments), sorted(list(nonempty_nsx_segments)))
            )
