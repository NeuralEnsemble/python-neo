...
neo_attributes = {
    ...
    "spiketrain": ["units", "copy", "sampling_rate", "t_start", "t_stop", "waveforms", "left_sweep"],
        ...
        for blk_idx, blk in enumerate(self.file.blocks):
            ...
            for grp_idx, group in enumerate(seg_groups):
                ...
                for da_idx, da in enumerate(group.data_arrays):
                    ...
                    props = [p for p in da.metadata.props if p.type != "ARRAYANNOTATION"]
                    ...
                    props = [p for p in da.metadata.props if p.type == "ARRAYANNOTATION"]
                    props_dict = self._filter_properties(props, "analogsignal")
                    sig_ann[stream_id]["__array_annotations__"].update(props_dict)
