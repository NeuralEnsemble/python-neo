...
class AxonaRawIO(BaseRawIO):
    ...
    def __init__(self, filename):
        ...
        if not self.set_file.exists():
            raise ValueError(
                f'Could not locate ".set" file. ' f'{self.filename.with_suffix(".set")} does not ' f"exist."
            )
    ...
    def _parse_header(self):
        ...
        unit_dtype = np.dtype([("spiketimes", ">i4"), ("samples", "int8", (50,))])
        ...
        params = {
            ...
            "set": {"filename": self.set_file, "header_encoding": "cp1252"},
            "unit": {"data_type": unit_dtype, "tetrode_ids": [], "header_encoding": "cp1252"},
        }
        ...
        if self.bin_file:
            ...
            num_tot_packets = int(self.bin_file.stat().st_size / bin_dict["bytes_packet"])
            ...
            self._raw_signals = np.memmap(
                ...
                mode="r",
                offset=self.file_parameters["bin"]["header_size"],
            )
        ...
        if self.tetrode_files:
            ...
            for i, tetrode_file in enumerate(self.tetrode_files):
                ...
                tdict["header_size"] = len(self.get_header_bstring(tetrode_file))
                ...
                wf_left_sweep = int(self.file_parameters["set"]["file_header"]["pretrigSamps"])
                ...
                sm = self.file_parameters["set"]["file_header"].get("Spike2msMode", -1)
            ...
            if ids:
                for key, value in units_dict[ids[0]].items():
                    ...
                    if all([key in units_dict[t] for t in ids]) and all([units_dict[t][key] == value for t in ids]):
    ...
    def _segment_t_stop(self, block_index, seg_index):
        ...
        if self.file_parameters["unit"]["tetrode_ids"]:
            ...
            if "duration" not in self.file_parameters["unit"]:
                raise ValueError("Can not determine common tetrode recording" "duration.")
    ...
    def _get_analogsignal_chunk(self, block_index, seg_index, i_start, i_stop, stream_index, channel_indexes):
        ...
        raw_signals = np.ndarray(shape=(num_samples, len(channel_indexes)), dtype=bin_dict["data_type"])
    ...
    def _rescale_spike_timestamp(self, spike_timestamps, dtype):
        ...
        spike_times /= self._to_hz(self.file_parameters["unit"]["timebase"], dtype=int)
        return spike_times
    ...
    def read_datetime(self):
        ...
        return datetime.datetime.strptime(date_str + ", " + time_str, "%d %b %Y, %H:%M:%S")
    ...
    def _get_channel_gain(self, bytes_per_sample=2):
        ...
        adc_fm = int(self.file_parameters["set"]["file_header"]["ADC_fullscale_mv"])
