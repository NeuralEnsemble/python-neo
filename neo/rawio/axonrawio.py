...
class AxonRawIO(BaseRawIO):
    ...
    def _parse_header(self):
        ...
        if version < 2.0:
            ...
            head_offset = (
                info["lDataSectionPtr"] * BLOCKSIZE
                + info["nNumPointsIgnored"] * sig_dtype.itemsize
            )
        ...
        self._raw_data = np.memmap(
            self.filename, dtype=sig_dtype, mode="r", shape=(totalsize,), offset=head_offset
        )
        ...
        for seg_index in range(nb_segment):
            ...
            self._raw_signals[seg_index] = self._raw_data[pos : pos + length].reshape(
                -1, nbchannel
            )
            pos += length
    ...
    def _segment_t_stop(self, block_index, seg_index):
        t_stop = (
            self._t_starts[seg_index] + self._raw_signals[seg_index].shape[0] / self._sampling_rate
        )
        return t_stop
    ...
    def read_raw_protocol(self):
        ...
        nSam = int(
            info["protocol"]["lNumSamplesPerEpisode"] / nADC
    ...
    with open(filename, "rb") as fid:
        ...
        if header["fFileVersionNumber"] < 2.0:
            ...
            for i in range(sections["ADCSection"]["llNumEntries"]):
                # read ADCInfo
                f.seek(
                    sections["ADCSection"]["uBlockIndex"] * BLOCKSIZE
                    + sections["ADCSection"]["uBytes"] * i
                )
                ADCInfo = {}
            ...
            for i in range(sections["TagSection"]["llNumEntries"]):
                f.seek(
                    sections["TagSection"]["uBlockIndex"] * BLOCKSIZE
                    + sections["TagSection"]["uBytes"] * i
                )
                tag = {}
                for key, fmt in TagInfoDescription:
                    val = f.read_f(fmt)
            ...
            for i in range(sections["DACSection"]["llNumEntries"]):
                # read DACInfo
                f.seek(
                    sections["DACSection"]["uBlockIndex"] * BLOCKSIZE
                    + sections["DACSection"]["uBytes"] * i
                )
                DACInfo = {}
            ...
            for i in range(sections["EpochSection"]["llNumEntries"]):
                # read EpochInfo
                f.seek(
                    sections["EpochSection"]["uBlockIndex"] * BLOCKSIZE
                    + sections["EpochSection"]["uBytes"] * i
                )
                EpochInfo = {}
