...
class AxographRawIO(BaseRawIO):
    ...
    def _get_analogsignal_chunk(self, block_index, seg_index, i_start, i_stop, channel_indexes):
        ...
        raw_signals = [
            self._raw_signals[seg_index][channel_index][slice(i_start, i_stop)]
    ...
    def _safe_to_treat_as_episodic(self):
        ...
        if self.info["format_ver"] < 3:
            self.logger.debug(
                "Cannot treat as episodic because old format " "contains insufficient metadata"
            )
            return False
        ...
        if "n_episodes" not in self.info:
            self.logger.debug(
                "Cannot treat as episodic because episode "
        ...
        if "group_header_info_list" not in self.info:
            self.logger.debug(
                "Cannot treat as episodic because group "
                "metadata is missing or could not be parsed"
            )
            return False
        if "trace_header_info_list" not in self.info:
            self.logger.debug(
                "Cannot treat as episodic because trace "
                "metadata is missing or could not be parsed"
            )
            return False
        ...
        all_groups_have_same_number_of_traces = (
            len(np.unique(list(n_traces_by_group.values()))) == 1
        )

        if not all_groups_have_same_number_of_traces:
            self.logger.debug(
                "Cannot treat as episodic because groups differ " "in number of traces"
            )
            return False
        ...
        if n_traces_per_group != self.info["n_episodes"]:
            self.logger.debug(
                "Cannot treat as episodic because n_episodes "
        ...
        all_groups_have_uniform_signal_parameters = np.all(
            list(group_has_uniform_signal_parameters.values())
        )

        if not all_groups_have_uniform_signal_parameters:
            self.logger.debug(
                "Cannot treat as episodic because some groups "
    ...
    def _convert_to_multi_segment(self):
        ...
        self.header["signal_channels"] = self.header["signal_channels"].reshape(
            self.info["n_episodes"], -1
        )[0]
    ...
    def _scan_axograph_file(self):
        ...
        with open(self.filename, "rb") as fid:
            ...
            assert header_id in ["AxGr", "axgx"], 'not an AxoGraph binary file! "{}"'.format(
                self.filename
            )
            ...
            if header_id == "AxGr":
                ...
                raise NotImplementedError(
                    'unimplemented file header identifier "{}"!'.format(header_id)
                )
            ...
            for i in range(n_cols):
                ...
                if format_ver == 1 or format_ver == 2:
                    ...
                    raise NotImplementedError(
                        'unimplemented file format version "{}"!'.format(format_ver)
                    )
                ...
                if format_ver == 1:
                    ...
                    if i == 0:
                        ...
                        self.logger.debug(
                            "interval: {}, freq: {}".format(increment, 1 / increment)
                        )
                        self.logger.debug(
                            "start: {}, end: {}".format(
                                first_value, first_value + increment * (n_points - 1)
                            )
                        )
                ...
                if i == 0:
                    ...
                    if max_frac_step_deviation > tolerance:
                        ...
                        raise ValueError(
                            "first data column (assumed to be " "time) is not regularly spaced"
                        )
            ...
            try:
                ...
                assert n_groups == len(
                    group_ids
                ), "expected group_ids to have length {}: {}".format(n_groups, group_ids)
                ...
                if format_ver == 5:
                    ...
                    self.logger.debug(
                        "old_unknown_episode_list: {}".format(old_unknown_episode_list)
                    )
                ...
                for i in font_categories:
                    ...
                    assert font_settings_info["setting1"] in [FONT_BOLD, FONT_NOT_BOLD], (
                        ...
                        "({}) or FONT_NOT_BOLD ({})".format(
                            font_settings_info["setting1"], FONT_BOLD, FONT_NOT_BOLD
                        )
                    )
                    ...
                    font_settings_info["italics"] = bool(
                        font_settings_info["setting2"] & FONT_ITALICS
                    )
                    font_settings_info["underline"] = bool(
                        font_settings_info["setting2"] & FONT_UNDERLINE
                    )
                    font_settings_info["strikeout"] = bool(
                        font_settings_info["setting2"] & FONT_STRIKEOUT
                    )
                ...
                for event_label, event_index in zip(event_labels, raw_event_timestamps):
                    ...
                    event_list.append(
                        {"title": event_label, "index": event_index, "time": event_time}
                    )
