...
class Spike2RawIO(BaseRawIO):
    ...
    def __init__(
        self, filename="", take_ideal_sampling_rate=False, ced_units=True, try_signal_grouping=True
    ):
    ...
    def _parse_header(self):
        ...
        with open(self.filename, "rb") as fid:
            ...
            for chan_id in range(info["channels"]):
                ...
                if chan_info["blocks"] == 0:
                    ...
                    chan_info["t_start"] = (
                        float(block_info["start_time"])
                        * float(info["us_per_time"])
                        * float(info["dtime_base"])
                    )
        ...
        for chan_id, chan_info in enumerate(self._channel_infos):
            ...
            for b in range(chan_info["blocks"]):
                ...
                data_blocks.append(
                    (ind, block_info["items"], 0, block_info["start_time"], block_info["end_time"])
                )
        ...
        for chan_id, chan_info in enumerate(self._channel_infos):
            if chan_info["kind"] in [1, 9]:
                ...
                if sig_size > 0:
                    interval = int(
                        np.round(get_sample_interval(info, chan_info) / self._time_factor)
                    )
                    # detect gaps
                    inter_block_sizes = (
                        data_blocks["start_time"][1:] - data_blocks["end_time"][:-1]
                    )
        ...
        for chan_id, chan_info in enumerate(self._channel_infos):
            ...
            if chan_info["kind"] in [1, 9]:
                ...
                group_id = 0
                sig_channels.append(
                    (name, chan_id, sampling_rate, sig_dtype, units, gain, offset, group_id)
                )
    ...
    def _get_internal_timestamp_(
        self, seg_index, chan_id, t_start, t_stop, other_field=None, marker_filter=None
    ):
        chan_info = self._channel_infos[chan_id]
        # data_blocks = self._by_seg_data_blocks[chan_id][seg_index]
        data_blocks = self._all_data_blocks[chan_id]
    ...
    def _spike_count(self, block_index, seg_index, unit_index):
        ...
        lim0 = self._seg_t_starts[seg_index]
        lim1 = self._seg_t_stops[seg_index]
        return self._count_in_time_slice(
            seg_index, chan_id, lim0, lim1, marker_filter=marker_filter
        )
    ...
    def _get_event_timestamps(self, block_index, seg_index, event_channel_index, t_start, t_stop):
        ...
        if chan_info["kind"] == 5:
            ...
            timestamps, labels = self._get_internal_timestamp_(
                seg_index, chan_id, t_start, t_stop, other_field="label"
            )
        else:
            timestamps = self._get_internal_timestamp_(
                seg_index, chan_id, t_start, t_stop, other_field=None
            )
...
def get_sample_interval(info, chan_info):
    ...
    if info["system_id"] in [1, 2, 3, 4, 5]:  # Before version 5
        sample_interval = (
            int(chan_info["divide"]) * info["us_per_time"] * info["time_per_adc"]
        ) * 1e-6
    else:
