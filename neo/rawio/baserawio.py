...
class BaseRawIO(object):
    ...
    def parse_header(self):
        ...
        This must contain self.header['nb_block']
        self.header['nb_segment'] self.header['signal_channels']
        self.header['units_channels'] self.header['event_channels']
    ...
    def _generate_minimal_annotations(self):
        """Helper function that generate a nested dict of all annotations. must
        be called when theses are Ok:
    ...
    def segment_t_start(self, block_index, seg_index):
        """Global t_start of a Segment in s.

        shared by all objects except for AnalogSignal.
    ...
    def _check_common_characteristics(self, channel_indexes):
        ...
        characteristics (**sampling_rate/t_start/size**) Usefull only
        when RawIO propose differents channels groups with differents
    ...
    def _get_channel_indexes(self, channel_indexes, channel_names, channel_ids):
        """select channel_indexes from
        channel_indexes/channel_names/channel_ids depending which is not
        None."""
    ...
    def get_analogsignal_chunk(self,
        ...
        raw_chunk = self._get_analogsignal_chunk(block_index, seg_index, i_start, i_stop,
                                                 channel_indexes)
    ...
    def get_spike_timestamps(self,
                             ...
                             t_start=None,
                             t_stop=None):
        """The timestamp is as close to the format itself. Sometimes
        float/int32/int64. Sometimes it is the index on the signal but not
        always. The conversion to second or index_on_signal is done outside
        here.
