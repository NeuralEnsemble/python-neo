...
def test_BlackrockRawIO():
    ...
    assert reader.get_nb_block()==1
    
    assert reader.get_nb_segment(0)==1
    
    assert reader.get_nb_analogsignal(0,0)==1
    #~ get_nb_analogsignal
    ...
    for i in range(nb_chunk):
        ...
        sig_chunk = reader.get_analogsignal_chunk(block_index=0, seg_index=0, anasig_index=0,
