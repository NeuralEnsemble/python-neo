...
class OpenEphysRawIO(BaseRawIO):
    ...
    def _parse_header(self):
        ...
        for seg_index, oe_index in enumerate(oe_indices):
            ...
            for chan_index, continuous_filename in enumerate(info["continuous"][oe_index]):
                ...
                data_chan = np.memmap(
                    fullname, mode="r", offset=HEADER_SIZE, dtype=continuous_dtype, shape=(size,)
                )
            ...
            if not all(all_sigs_length[0] == e for e in all_sigs_length) or not all(
                all_first_timestamps[0] == e for e in all_first_timestamps
            ):

                self.logger.warning(
                    "Continuous files do not have aligned timestamps; "
        ...
        if len(info["spikes"]) > 0:
            ...
            for seg_index, oe_index in enumerate(oe_indices):
                ...
                for spike_filename in info["spikes"][oe_index]:
                    ...
                    data_spike = np.memmap(
                        fullname, mode="r", offset=HEADER_SIZE, dtype=spikes_dtype
                    )
    ...
    def _segment_t_stop(self, block_index, seg_index):
        return (
            self._sig_timestamp0[seg_index] + self._sig_length[seg_index]
    ...
    def _get_event_timestamps(self, block_index, seg_index, event_channel_index, t_start, t_stop):
        ...
        labels = np.array(
            [
                "{}#{}#{}".format(int(d["event_type"]), int(d["processor_id"]), int(d["chan_id"]))
                for d in subdata
            ],
            dtype="U",
        )
