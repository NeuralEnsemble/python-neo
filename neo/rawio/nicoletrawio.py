...
class NicoletRawIO(BaseRawIO):
    ...
    def _parse_header(self):
        ...
        self.header["signal_buffers"] = np.array(["Signals", "0"], dtype=_signal_buffer_dtype)
        self.header["signal_channels"] = self._create_signal_channels(_signal_channel_dtype)
        self.header["signal_streams"] = np.array(
            [(f"Signals {signal_id}", signal_id, "0") for signal_id in self.signal_streams.values()],
    ...
    def _get_main_index(self):
        ...
        with open(self.filename, "rb") as fid:
            while current_index < self.qi["n_entries"]:
                ...
                var = self.read_as_list(fid, [("var", "uint64", int(3 * nr_index))])
            ...
            for i in range(self.n_dynamic_packets):
                ...
                for j in range(len(dynamic_packet_instances)):
                    ...
                    if (internal_offset <= (current_target_start)) & (
                        ...
                        stop_at = min(
                            start_at + remaining_data_to_read, internal_offset + current_instance["section_l"]
                        )
                        read_length = stop_at - start_at
                        file_pos_start = current_instance["offset"] + start_at - internal_offset
                        fid.seek(int(file_pos_start))
                        data_part = self.read_as_list(fid, [("data", "uint8", read_length)])
    ...
    def _get_patient_guid(self):
        ...
        with open(self.filename, "rb") as fid:
            ...
            if patient_info["n_bstr"] != 0:
                str_setup = self.read_as_list(fid, [("setup", "uint64", int(patient_info["n_bstr"] * 2))])
                for i in range(0, int(patient_info["n_bstr"] * 2), 2):
                    id_temp = str_setup[i]
                    value = "".join(
                        [self.read_as_list(fid, [("value", "S2")]) for _ in range(int(str_setup[i + 1]) + 1)]
                    ).strip()
                    patient_info[self.INFO_PROPS[int(id_temp) - 1]] = value
    ...
    def _get_channel_info(self):
        ...
        with open(self.filename, "rb") as fid:
            ...
            fid.seek(152, 1)
            channel_structure = channel_structure | self.read_as_dict(fid, channel_structure_structure[1])
            fid.seek(488, 1)
            ...
            for i in range(n_index[1]):
                ...
                info = self.read_as_dict(fid, channel_properties_structure)
                fid.seek(128, 1)
                if info["on"]:
    ...
    def _get_ts_properties(self, ts_packet_index=0):
        ...
        for ts_packet in ts_packets:
            ...
            for _ in range(elems):
                ...
                active_sensor = self._transform_ts_properties(
                    ts_packet["data"][(offset + internal_offset) : top_range], np.uint16
                )
                internal_offset = internal_offset + self.TSLABELSIZE
                top_range = offset + internal_offset + 8
                ref_sensor = self._transform_ts_properties(
                    ts_packet["data"][(offset + internal_offset) : top_range], np.uint16
                )
                internal_offset += 64
                ...
                eeg_offset, internal_offset = self._read_ts_properties(
                    ts_packet["data"], offset, internal_offset, np.float64
                )
                offset += 552
    ...
    def _get_montage(self):
        ...
        with open(self.filename, "rb") as fid:
            ...
            montage_info = self.read_as_dict(fid, montage_info_structure[0])
            fid.seek(640, 1)
            montage_info = montage_info | self.read_as_dict(fid, montage_info_structure[1])

            for i in range(montage_info["n_derivations"]):
                montage = montage_info | self.read_as_dict(fid, montage_info_structure[2])
                fid.seek(264, 1)
            ...
            display = self.read_as_dict(fid, display_structure[0])
            fid.seek(640, 1)
            display = display | self.read_as_dict(fid, display_structure[1])
    ...
    def _get_buffer_descriptions(self):
        ...
        for seg_index, segment in enumerate(self.segments_properties):
            current_samplingrate = segment["sampling_rates"][
                0
            ]  # Non signal-stream specific, just take the sampling rate of the first channel
            skip_values = (
                [0] + list(np.cumsum([(segment["duration"].total_seconds()) for segment in self.segments_properties]))
            )[seg_index] * current_samplingrate
            ...
            section_lengths = [0] + list(
                np.cumsum([int(index["section_l"] / 2) for j, index in enumerate(self.main_index) if j in all_sections])
            )
    ...
    def _generate_additional_annotations(self):
        ...
        for block_index in range(self.header["nb_block"]):
            ...
            bl_annotations["duration"] = sum(
                [properties["duration"].total_seconds() for properties in self.segments_properties]
            )
    ...
    def _get_analogsignal_chunk(
        ...
        if seg_index >= self.header["nb_segment"][block_index]:
            raise IndexError(
                f"Segment Index out of range. There are {self.header['nb_segment'][block_index]} segments for block {block_index}"
            )
    ...
    def _get_signal_size(self, block_index: int = 0, seg_index: int = 0, stream_index: int = 0):
        ...
        """
        return max(self.get_nr_samples(block_index=block_index, seg_index=seg_index, stream_index=stream_index))
    ...
    def _get_index_instances(self, id_str="", tag=""):
        ...
        try:
            ...
            idx_instance = [{"section_idx": 0, "offset": 0, "block_l": 0, "section_l": 0}]
        return idx_instance
