...
class BCI2000RawIO(BaseRawIO):
    ...
    def _parse_header(self):
        ...
        for chan_ix in range(file_info["SourceCh"]):
            ch_name = (
                param_defs["ChannelNames"]["value"][chan_ix]
                if "ChannelNames" in param_defs
                and param_defs["ChannelNames"]["value"] is not np.nan
                else "ch" + str(chan_ix)
            )
            chan_id = chan_ix + 1
        ...
        self.raw_annotations["blocks"][0].update(
            {"file_info": file_info, "param_defs": param_defs}
        )
        ...
        try:
            self._global_time = time.mktime(
                time.strptime(param_defs["StorageTime"]["value"], time_formats[0])
            )
        except:
            self._global_time = time.mktime(
                time.strptime(param_defs["StorageTime"]["value"], time_formats[1])
            )
        ...
        self._read_info = {
            ...
            "sample_dtype": {"int16": np.int16, "int32": np.int32, "float32": np.float32}.get(
                file_info["DataFormat"]
            ),
        ...
        self._read_info["n_samps"] = int(
            (os.stat(self.filename).st_size - file_info["HeaderLen"])
        ...
        self._memmap = np.memmap(
            self.filename,
            dtype=self._read_info["line_dtype"],
            offset=self._read_info["header_len"],
            mode="r",
        )
        ...
        if self._my_events is None:
            ...
            for s_ix, sd in enumerate(self.raw_annotations["event_channels"]):
                ...
                if sd["name"] not in ["SourceTime", "StimulusTime"]:
                    ...
                    view_type = {1: np.int8, 2: np.int16, 4: np.int32, 8: np.int64}.get(
                        n_max_bytes
                    )
                    ...
                    state_vec = np.pad(
                        masked_byte_array, (0, n_max_bytes - nbytes), "constant"
                    ).view(dtype=view_type)
...
def parse_bci2000_header(filename):
    ...
    with open(filename, "rb") as fid:
        ...
        while True:
            ...
            param_def.update({"value": param_value, "units": units, "dtype": dtype})
