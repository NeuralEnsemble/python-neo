...
class BCI2000RawIO(BaseRawIO):
    ...
    def _parse_header(self):
        ...
        self.raw_annotations["blocks"][0].update({"file_info": file_info, "param_defs": param_defs})
        ...
        self._read_info["n_samps"] = int(
            (os.stat(self.filename).st_size - file_info["HeaderLen"]) / np.dtype(self._read_info["line_dtype"]).itemsize
        )

        # memmap is fast so we can get the data ready for reading now.
        self._memmap = np.memmap(
            self.filename, dtype=self._read_info["line_dtype"], offset=self._read_info["header_len"], mode="r"
        )
