    ...
    def _parse_header(self):
        ...
        for filename in sorted(os.listdir(self.dirname)):
            ...
            if ext == 'ncs':
                ...
                data = np.memmap(
                    filename, dtype=dtype, mode='r', offset=HEADER_SIZE)
...
txt_header_keys = [
    ...
    ('TimeCreated', '', None),
    ('TimeClosed', '', None),
    ('ApplicationName Cheetah', 'version',
     None),    #used  possibilty 2 for version
...
def read_txt_header(filename):
    ...
    original_filename = re.search(filename_regex,
                                  txt_header).groupdict()['filename']
...
ncs_dtype = [('timestamp', 'uint64'), ('channel_id', 'uint32'), ('sample_rate',
                                                                 'uint32'),
...
def get_nse_or_ntt_dtype(info, ext):
    ...
    """
    dtype = [('timestamp', 'uint64'), ('channel_id', 'uint32'),
             ('unit_id', 'uint32'), ('features', 'int32', (8, ))]
