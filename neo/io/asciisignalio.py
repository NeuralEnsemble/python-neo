        ...
        if self.signal_group_mode == 'all-in-one':
            ...
            if sampling_rate is None:
                ...
                ana_sig = AnalogSignal(signal * self.units, sampling_rate=sampling_rate,
                                       t_start=t_start,
                                       name='multichannel')
                if self.timecolumn is not None and self.usecols is not None:
                    ana_sig.array_annotate(channel_index=list(self.usecols).remove(self.usecols[self.timecolumn]) or np.arange(signal.shape[1]))
                else:
