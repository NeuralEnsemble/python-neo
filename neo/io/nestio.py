...
class NestIO(BaseIO):
    ...
    def __read_analogsignals(
        ...
        sampling_period = self._check_input_sampling_period(
            sampling_period, time_column, time_unit, data
        )
        analogsignal_list = []
        ...
        for i in gid_list:
            ...
            for v_id, value_column in enumerate(value_columns):
                ...
                assert (
                    analogsignal_list[-1].t_stop
                    == anasig_start_time + len(signal) * sampling_period
                )
        return analogsignal_list

    def __read_spiketrains(
        self, gdf_id_list, time_unit, t_start, t_stop, id_column, time_column, **args
    ):
        """
        ...
        if id_column == time_column:
            raise ValueError(
                "One or more columns have been specified to " "contain the same data."
            )
        ...
        if id_column is not None:
            ...
            for nid in gdf_id_list:
                selected_ids = self._get_selected_ids(
                    nid, id_column, time_column, t_start, t_stop, time_unit, data
                )
                times = data[selected_ids[0] : selected_ids[1], time_column]
                spiketrain_list.append(
                    SpikeTrain(
                        times, units=time_unit, t_start=t_start, t_stop=t_stop, id=nid, **args
                    )
                )
        ...
        #  spike train with id=None
            ...
            spiketrain_list = [
                SpikeTrain(train, units=time_unit, t_start=t_start, t_stop=t_stop, id=None, **args)
            ]
    ...
    def _check_input_gids(self, gid_list, id_column):
        ...
        if gid_list != [None] and id_column is None:
            raise ValueError(
                "Specified neuron IDs to be %s, but no ID column " "specified." % gid_list
            )
        ...
        if sampling_period is None:
            if time_column is not None:
                ...
                if len(data_sampling) > 1:
                    raise ValueError(
                        "Different sampling distances found in " "data set (%s)" % data_sampling
                    )
                else:
                    dt = data_sampling[0]
            else:
                raise ValueError(
                    "Can not estimate sampling rate without time " "column id provided."
                )
    ...
    def _get_conditions_and_sorting(self, id_column, time_column, gid_list, t_start, t_stop):
        ...
        if time_column is not None:
            ...
            warnings.warn(
                "Ignoring t_start and t_stop parameters, because no " "time column id is provided."
            )
        if sorting_column == []:
            sorting_column = None
        else:
    ...
    def _get_selected_ids(self, gid, id_column, time_column, t_start, t_stop, time_unit, data):
        ...
        if id_column is not None:
            gid_ids = np.array(
                [
                    np.searchsorted(data[:, 0], gid, side="left"),
        ...
        if time_column is not None:
            id_shifts[0] = np.searchsorted(
                gid_data[:, 1], t_start.rescale(time_unit).magnitude, side="left"
            )
            id_shifts[1] = (
                np.searchsorted(gid_data[:, 1], t_stop.rescale(time_unit).magnitude, side="left")
                - gid_data.shape[0]
            )
    ...
    def read_segment(
        ...
        if isinstance(gid_list, tuple):
            if gid_list[0] > gid_list[1]:
                raise ValueError(
                    "The second entry in gid_list must be " "greater or equal to the first entry."
                )
    ...
    def read_spiketrain(
        ...
        **args
        ...
        if gdf_id is None and id_column is not None:
            raise ValueError(
                "No neuron ID specified but file contains "
                "neuron IDs in column " + str(id_column) + "."
            )

        return self.__read_spiketrains(
            [gdf_id], time_unit, t_start, t_stop, id_column, time_column, **args
        )[0]
...
class ColumnIO:
    ...
    def get_columns(
        self, column_ids="all", condition=None, condition_column=None, sorting_columns=None
    ):
        """
        ...
        if column_ids is not None:
            if max(column_ids) >= len(self.data) - 1:
                raise ValueError(
                    "Can not load column ID %i. File contains "
                    "only %i columns" % (max(column_ids), len(self.data))
                )
        ...
        if condition and (condition_column is None):
            ...
            warnings.warn(
                "Condition column ID provided, but no condition "
