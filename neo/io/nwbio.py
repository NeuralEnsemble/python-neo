...
################################################################################
            ...
            for obj in self._file.get_acquisition:
                obj.segment = segment
            ...
            print("segment = ", segment)
            print("block.segments.append(segment) = ", block.segments.append(segment))
################################################################################
...
#         subtype = timeseries.attrs['ancestry'][-1]
        ...
        for i in self._file.acquisition:
            ...
            if dtype.type is np.string_:
                ...
                #print("units = ", units)
            current_shape = self._file.get_acquisition(i).data.shape[0] # sample number
            #print("current_shape = ", current_shape)
            times = np.zeros(current_shape)
            for j in range(0, current_shape):
