...
class KlustaKwikIO(BaseIO):
    ...
    def __init__(self, filename, sampling_rate=30000.0):
    ...
    def read_block(self, lazy=False):
        ...
        for group in sorted(self._fetfiles.keys()):
            ...
            for unit_id in sorted(unique_unit_ids):
                # Initialize the unit
                u = Unit(
                    name=("unit %d from group %d" % (unit_id, group)), index=unit_id, group=group
                )
    ...
    def _load_unit_id(self, clufilename):
        ...
        try:
            ...
            raise ValueError("Could not convert cluster name to integer in %s" % clufilename)
        ...
        if len(np.unique(cluster_ids)) != nbClusters:
            logging.warning(
                "warning: I got %d clusters instead of %d in %s"
    ...
    def write_block(self, block):
        ...
        for seg in block.segments:
            ...
            for st in seg.spiketrains:
                ...
                # Convert to samples
                spike_times_in_samples = np.rint(np.array(st) * sr).astype(np.int)
                ...
                if n_features != all_features.shape[1]:
                    raise ValueError(
                        ...
                        + "supposed to be %d but I got %d" % (n_features, all_features.shape[1])
                    )
