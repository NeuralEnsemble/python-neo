...
class BlackrockIO(BaseIO):
    ...
    Upon initialization, the class is linked to a set of Blackrock files. In
    this process, basic information about the file(s) is obtained, and stored
    in the class attributes. Data can be read as a neo Block object using the
    read_block method.
    ...
    # Analog files are stored in files .ns0 - .ns6, spikes and events are
    # stored in .nev:
    ...
    # Only those nsX files are listed which are currently documented by
    # Blackrock Systems, i.e., ns1-ns6
    ...
    def __init__(
        ...
        several features (e.g., unit IDs) are already read from these data
        files for later use.
        ...
        Examples:
            a=BlackrockIO('myfile')
                ...
                Loads a set of file consisting of files myfile.nev,
                myfile.ns1, myfile.ns1,...,myfile.ns6
    ...
    def __read_nsx_header(self, filehandle, nsx):
        ...
        Args:
            ...
            nsx (int):
                Integer ID x of the .nsX file under consideration. Determines
                in which parameters_nsx dictionary the header information is
                read.
        ...
        if fileheader == 'NEURALSG':
            ...
            self.parameters_nsx[nsx]['AnalogTimeResolution'] = \
                self.analog_res[nsx]
            ...
            self.parameters_nsx[nsx]['Version'] = \
                str(major) + '.' + str(minor)
            ...
            # unsigned int 32bit, little endian
            (self.__file_nsx_header_end_pos[nsx],) = \
                struct.unpack('<I', filehandle.read(4))

            # Read label string and remove 0's
            self.parameters_nsx[nsx]['Label'] = \
                filehandle.read(16).split('\x00', 1)[0]

            # Read comments and remove 0's
            self.parameters_nsx[nsx]['Comments'] = \
                filehandle.read(256).split('\x00', 1)[0]
            ...
            # Read resolution
            # unsigned int 32bit, little endian
            (self.parameters_nsx[nsx]['EventTimeResolution'],) = \
                struct.unpack('<I', filehandle.read(4))
            ...
            self.parameters_nsx[nsx]['Date'] = str(date_time[1]) + '/' + \
                ...
                weekdays[date_time[2] - 1]
            self.parameters_nsx[nsx]['Time'] = str(date_time[4]) + ':' + \
                str(date_time[5]) + ':' + str(date_time[6]) + ':' + \
                str(date_time[7])
            ...
            for channel_i in range(self.num_channels_nsx[nsx]):
                ...
                # Read label string and remove 0's
                self.parameters_nsx_electrodes[nsx][el_id]['ChannelLabel'] = \
                    filehandle.read(16).replace('\x00', '')

                # Read physical connector (bank A-D)
                # unsigned byte 8bit, little endian
                (self.parameters_nsx_electrodes[nsx][el_id][
                    'ConnectorID'],) = struct.unpack('<B', filehandle.read(1))

                # Read connector pin (pin number on connector, 1-37)
                # unsigned byte 8bit, little endian
                (self.parameters_nsx_electrodes[nsx][el_id][
                    'ConnectorPin'],) = struct.unpack('<B', filehandle.read(1))

                # Minimal digital value (e.g., -8192)
                # unsigned short 16bit, little endian
                (self.parameters_nsx_electrodes[nsx][el_id][
                    'MinDigiValue'],) = struct.unpack('<H', filehandle.read(2))

                # Maximum digital value (e.g., 8192)
                # unsigned short 16bit, little endian
                (self.parameters_nsx_electrodes[nsx][el_id][
                    'MaxDigiValue'],) = struct.unpack('<H', filehandle.read(2))
                ...
                # unsigned long  32bit, little endian
                (self.parameters_nsx_electrodes[nsx][el_id][
                    'LoFreqCorner'],) = struct.unpack('<I', filehandle.read(4))

                # Read low frequency filter order
                # unsigned long  32bit, little endian
                (self.parameters_nsx_electrodes[nsx][el_id][
                    'LoFreqOrder'],) = struct.unpack('<I', filehandle.read(4))
    ...
    def __read_nev_header(self, filehandle):
        ...
        # Read end position of extended headers, i.e. index of first data
        # packet
        # unsigned long 32bit, little endian
        (self.__file_nev_ext_header_end_pos,) = \
            struct.unpack('<I', filehandle.read(4))
        ...
        self.parameters_nev['Date'] = str(date_time[1]) + '/' + \
            str(date_time[3]) + '/' + str(date_time[0]) + ' ' + \
            weekdays[date_time[2] - 1]
        self.parameters_nev['Time'] = str(date_time[4]) + ':' + \
            str(date_time[5]) + ':' + str(date_time[6]) + ':' + \
            str(date_time[7])
        self.parameters_nev['DateTime'] = datetime.datetime(
            year=date_time[0], month=date_time[1], day=date_time[3],
            hour=date_time[4], minute=date_time[5], second=date_time[6],
            microsecond=date_time[7])

        # Read comments and remove 0's
        self.parameters_nev['Application'] = \
            filehandle.read(32).split('\x00', 1)[0]

        # Read comments and remove 0's
        self.parameters_nev['Comments'] = \
            filehandle.read(256).split('\x00', 1)[0]
    ...
    def __read_nev_ext_header(self, filehandle):
        ...
        Args:
            filehandle (file object):
                Handle to the already opened .nev file. The file pointer is
                expected to be at the starting position of the first header,
                and the variable self.__num_ext_header should contain the total
                number of extended headers from the basic header block.
        ...
        for _ext_block_i in range(0, self.__num_ext_header):
            ...
            if fileextheader == 'ARRAYNME':
                ...
                self.parameters_nev['ArrayName'] = \
                    filehandle.read(24).replace('\x00', '')

            elif fileextheader == 'ECOMMENT':
                # Read extended comment and remove 0's
                self.parameters_nev['ExtendedComment'] = \
                    filehandle.read(24).replace('\x00', '')
                ...
                self.parameters_nev_electrodes[el_id]['Label'] = \
                    filehandle.read(16).replace('\x00', '')
    ...
    def _associate(self, sessionname, nsx_override=None, nev_override=None):
        ...
        if not os.path.exists(filename_nev):
            ...
            # Make sure the number of data bytes is a multiple of the size of
            # data packets
            if not (self.__file_nev_end_pos -
                    self.__file_nev_ext_header_end_pos) % \
                    self.__packet_bytes == 0:
                raise IOError("Number of data packets in .nev file \
            ...
            # timestamp:
            #   unsigned long 32bit, little endian
            # packet id (0 for experiment event, or electrode id):
            #   unsigned short 16bit, little endian
            # classification or reason:
            #   unsigned byte 8bit, little endian
            #     packet id =0:
            #       insertion reason (bit 0=digital, bits 1-5 analog channels
            #     packet id >0:
            #       classifier (0=unclass, 1-16 units 1-16, 255 noise)
            # reserved:
            #   unsigned byte 8bit, little endian
            # digital and analog values or waveform
            #   packet id =0:
            #     digital input:  unsigned short 16bit, little endian
            ...
            #   packet id >0:
            #     waveform: (__packet_bytes-8) unsig. bytes 8bit, little endian
            #
            # All arrays are converted to dtype 'int', which makes subsequent
            # operations considerably faster, at the cost of slightly increased
            # memory usage.
            buf = np.frombuffer(filebuffer, count=-1, dtype='<I').reshape(
                (self.__num_packets_nev, self.__packet_bytes / 4), order='C')
            self._event_timestamps = buf[:, 0].astype('int')

            buf = np.frombuffer(filebuffer, count=-1, dtype='<H').reshape(
                (self.__num_packets_nev, self.__packet_bytes / 2), order='C')
            ...
            buf = np.frombuffer(filebuffer, count=-1, dtype='<B').reshape(
            ...
            (self._comment_index,) = \
                np.nonzero(self._event_packet_id == 0xffff)
            ...
            for marker_idx in self._comment_index:
                self._comment_rgba_color[marker_idx] = \
                    buf[marker_idx, 2].astype('int')
            ...
            for marker_idx in self._comment_index:
                ...
                if charset == 0:
                    # ANSI ASCII: Remove unprintable characters
                    # (replace by 0 to end string)
                    ...
                    # UTF-16
                    self._comment[marker_idx] = unicode(buf[
            ...
            (self._digital_marker_index,) = np.nonzero(np.logical_and(
                ...
                np.bitwise_and(self._event_class_or_reason, 1)))
            ...
            self.nev_unit = pq.CompoundUnit(
                '1.0/' + str(self.timestamp_res) + '*s')
    ...
    def get_unit_ids(self, electrode):
        '''
        Returns a list of neuron IDs recorded on a specific electrode. Neuron
        ID 0 corresponds to unsorted neurons, ID 255 is the 'noise' channel.
        IDs 1-16 are sorted neurons. (cf. Blackrock IO manual)
    ...
    def get_max_time(self):
        ...
        Returns:
            Quantity:
                ...
                Note that this notation is compatible with pythonic indexing:
                If the last sample n of an analog signal is the last sample, or
                if the last time stamp n of spikes/markers is the last sample,
        ...
        for nsx_i in self.nsx_avail:
            last_nsx_time = \
                (self.__num_packets_nsx[nsx_i]) * self.nsx_unit[nsx_i]
    ...
    def read_block(
        ...
        Args:
            lazy (bool):
                If True, loads the neo block structure without the following
                data:
            ...
            n_starts (list):
                ...
                specified as None, the beginning of the file is used.
                Default: [None].
            n_stops (list):
                ...
                corresponding to the stop time is not returned.
                Default: [None].
            channel_list (list):
                List of channels to consider in loading (Blackrock channel
                IDs). The neural data channels are 1 - 128. The analog inputs
                are 129 - 255. If an empty list is specified, all channels are
                loaded. If None is specified, no channels are considered.
                Default: [].
            nsx (list):
                List of integers X (between 0 and 9) specifying the .nsX file
                to load analog data from channels in channel_list from. If an
                empty list is given, all available nsX files are loaded. If
                None is specified, no analog data is read.
                Blackrock defined sampling frequencies:
                    .ns6 - 30000 Hz (no digital filter)
                    .ns5 - 30000 Hz
                    .ns4 - 10000 Hz
                    .ns3 -  2000 Hz
            ...
            units (list or dictionary):
                ...
                contains a list of unit IDs to load from channel N (but only
                channels in channel_list are loaded). If None is specified, no
                units are loaded.
        ...
        Returns:
            neo.Block
                ...
                RecordingChannelGroup
                    For each recording channel, one RecordingChannelGroup
                    object is created.
                    Attributes:
                        name: string of the form "Channel i"
                        file_origin: session name
                        rec_datetime: date and time
                        channel_indexes: numpy array of one element, which is
                ...
                AnalogSignal
                    For each loaded analog signal, one AnalogSignal object is
                    created per Segment.
                    Attributes:
                        name: string of the form
                            "Analog Signal Segment i, Channel j, NSX x"
                        file_origin: session name
                    Annotations:
                ...
                SpikeTrain
                    ...
                    Waveforms of spikes are inserted into the waveforms
                    property of the respective SpikeTrain objects. Individual
                    Spike objects are not used.
                    Attributes:
                        name: string of the form "Segment i, Channel j, Unit u"
                        file_origin: session name
                        dtype: int (original time stamps are save in units of
                            nev_unit)
                ...
                EventArray
                    For each time-stamped comment in the data file
                    (version >=2.3) one event is created.
                    Attributes:
                    ...
                    Annotations:
                        ...
                        comment_rgba_color (int): color of the comment in RGBA
                            format (4 bytes lumped in one integer)

                EventArray
                    Per Segment, for digital or analog markers with a common
                    ID, one EventArray is created containing the time stamps of
                    all occurrences of that specific marker.
                    Attributes:
                        name: string of the form
                            "Digital Marker m" / "Analog Channel i Marker m"
                        file_origin: session name
                        dtype: int (original time stamps are save in units of
                            nev_unit)
                    ...
                    Annotations:
                        ...
                        analog_channel (int): For analog markers, determines
                            the respective channel (0-4)
        ...
        if units is None:
            ...
            for channel_i in channel_list:
                ...
                units[channel_i] = \
                    list(set(self.get_unit_ids(channel_i)) & set(unit_select))
                complete_unit_ids.update(units[channel_i])
            ...
            for channel_i in channel_list:
                ...
                # Keep only existing units
                units[channel_i] = list(set(
                    self.get_unit_ids(channel_i)) & set(units[channel_i]))
                complete_unit_ids.update(units[channel_i])
        else:
            raise ValueError('Invalid specification of channel_list.')
        ...
        # recordings channel groups, and units
        rc = {}
        rcg = {}
        un = dict([[i, {}] for i in channel_list])
        for channel_i in channel_list:
            rc[channel_i] = neo.RecordingChannel(
                name="Channel " + str(channel_i),
                index=channel_i,
                file_origin=self.associated_fileset,
                rec_datetime=recdatetime)
            ...
            rcg[channel_i] = neo.RecordingChannelGroup(
                name="Channel " + str(channel_i),
        ...
        # Precalculate indices of those unit IDs that are going to be loaded
        # for speed-up
        for unit_i in complete_unit_ids:
            u_idx[unit_i] = self._event_class_or_reason == unit_i

        # If spike waveforms are requested, pre-read the nev file packets
        # for speed reasons
        ...
        for channel_i in channel_list:
            ...
            if waveforms:
                # Determine digital resolution of waveforms, and allocate
                # array accordingly
                datatypes = ["", "<b", "<h", "", "<i"]
                numbytes = self.__byte_per_waveform_sample[channel_i]

                # Pre-allocate waveform array: one row per spike, one column
                # per waveform sample
                if numbytes not in [1, 2, 4]:
                    raise IOError("Invalid number of bytes per \
                    waveform sample (" + str(numbytes) + ").")

                # Only read data again if data type is different from
                # last channel
                if last_numbytes == 0 or numbytes != last_numbytes:
                    # Read all packets at once for speed reasons and reshape
                    # to matrix of that channel
                    #  timestamp:
                    #    unsigned long 32bit, little endian
                    #  packet id (0 for experiment event, or electrode id):
                    #    unsigned short 16bit, little endian
            ...
            for (seg_i, n_start_i, n_stop_i) in \
                ...
                for analog_channel_i in xrange(5):
                    for marker_i in self._analog_marker_ids[analog_channel_i]:
                        # Extract all time stamps of analog markers in channel
                        # analog_channel_i
                ...
                for marker_i, marker_idx in enumerate(self._comment_index):
                    if t_idx[seg_i][marker_idx]:
                        ev = neo.EventArray(
                            ...
                            comment_rgba_color=self._comment_rgba_color[
                                marker_idx])
        ...
        for nsx_i in nsx:
            ...
            if float(self.parameters_nsx[nsx_i]['Version']) > 2.1:
                try:
                    ...
                    # than one block of LFP data!
                    # TODO: Implement this functionality instead of raising
                    # an exception
                finally:
