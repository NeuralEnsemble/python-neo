...
class NeoMatlabIO(BaseIO):
    ...
    supported_objects = [Block, Segment, AnalogSignal, IrregularlySampledSignal, Epoch, Event, SpikeTrain]
    ...
    def create_struct_from_obj(self, ob):
        ...
        for childname in getattr(ob, "_child_containers", []):
            supported_containers = [subob.__name__.lower() + "s" for subob in self.supported_objects]
        ...
        for attrname in struct._fieldnames:
            ...
            if attrname in getattr(ob, "_child_containers", []):
                ...
                try:
                    ...
                    child = self.create_ob_from_struct(child_struct, classname_lower_to_upper[attrname[:-1]])
                    getattr(ob, attrname.lower()).append(child)
                else:
                    for c in range(child_len):
                        child = self.create_ob_from_struct(child_struct[c], classname_lower_to_upper[attrname[:-1]])
                        getattr(ob, attrname.lower()).append(child)
                continue
