...
class AnalogSignalProxy(BaseProxy):
    ...
    _necessary_attrs = (("sampling_rate", pq.Quantity, 0), ("t_start", pq.Quantity, 0))
    ...
    def __init__(self, rawio=None, global_channel_indexes=None, block_index=0, seg_index=0):
        ...
        assert (
            np.unique(sig_chans["sampling_rate"]).size == 1
        ...
        sigs_size = self._rawio.get_signal_size(
        ...
        self.t_start = (
            self._rawio.get_signal_t_start(block_index, seg_index, self._global_channel_indexes)
            * pq.s
        )

        # magnitude_mode='raw' is supported only if all offset=0
        # and all gain are the same
        support_raw_magnitude = np.all(sig_chans["gain"] == sig_chans["gain"][0]) and np.all(
            sig_chans["offset"] == 0.0
        )
        ...
        if len(sig_chans) == 1:
            ...
            d = self._rawio.raw_annotations["blocks"][block_index]["segments"][seg_index][
                "signals"
            ][self._global_channel_indexes[0]]
    ...
    def load(
        self, time_slice=None, strict_slicing=True, channel_indexes=None, magnitude_mode="rescaled"
    ):
        """
        *Args*:
...
class SpikeTrainProxy(BaseProxy):
    ...
    _necessary_attrs = (("t_start", pq.Quantity, 0), ("t_stop", pq.Quantity, 0))
    _recommended_attrs = ()
    ...
    def __init__(self, rawio=None, unit_index=None, block_index=0, seg_index=0):
        ...
        nb_spike = self._rawio.spike_count(
            block_index=block_index, seg_index=seg_index, unit_index=unit_index
        )
        self.shape = (nb_spike,)
        ...
        ann = self._rawio.raw_annotations["blocks"][block_index]["segments"][seg_index]["units"][
            unit_index
        ]
    ...
    def load(
        self, time_slice=None, strict_slicing=True, magnitude_mode="rescaled", load_waveforms=False
    ):
        """
        *Args*:
            :time_slice: None or tuple of the time slice expressed with quantities.
        ...
        t_start, t_stop = consolidate_time_slice(
            time_slice, self.t_start, self.t_stop, strict_slicing
        )
        ...
        if load_waveforms:
            ...
            if magnitude_mode == "rescaled":
                ...
                waveforms = pq.Quantity(
                    float_wfs, units=self._wf_units, dtype="float32", copy=False
                )
...
class _EventOrEpoch(BaseProxy):
    ...
    def __init__(self, rawio=None, event_channel_index=None, block_index=0, seg_index=0):
        ...
        nb_event = self._rawio.event_count(
        ...
        ann = self._rawio.raw_annotations["blocks"][block_index]["segments"][seg_index]["events"][
            event_channel_index
        ]
        annotations.update(ann)
...
proxyobjectlist = [AnalogSignalProxy, SpikeTrainProxy, EventProxy, EpochProxy]
...
def ensure_signal_units(units):
    ...
    try:
        ...
        logger.warning(
            'Units "{}" can not be converted to a quantity. Using dimensionless '
...
def create_analogsignal_array_annotations(sig_annotations, global_channel_indexes):
    ...
    for ind in global_channel_indexes:
        keys = [
            k
            for k, v in sig_annotations[ind].items()
