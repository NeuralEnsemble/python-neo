...
class BlkIO(BaseIO):
    ...
    *Usage*:
    ...
    def __init__(
        self, file_name=None, units=None, sampling_rate=None, spatial_scale=None, **kwargs
    ):
    ...
    def read_block(self, lazy=False, **kargs):
        ...
        for k in range(1, nbin + 1):
            print("reading block")
            bin = np.arange(
                math.floor((k - 1 / nbin * nblocks) + 1), math.floor((k / nbin * nblocks) + 1)
            )
            sbin = bin.size
            for j in range(1, sbin + 1):
                ...
                for i in range(1, ncond + 1):
                    ...
                    a = [
                        (struct.unpack(struct_type, file.read(nbytes)))[0]
                        for m in range(ni * nj * nfr)
                    ]
                    a = np.reshape(
                        np.array(a, dtype=type_out, order="F"), (ni * nj, nfr), order="F"
                    )
