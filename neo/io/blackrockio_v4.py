...
from neo.core import Block, Segment, SpikeTrain, Unit, Event, ChannelIndex, AnalogSignal
...
class BlackrockIO(BaseIO):
    ...
    supported_objects = [Block, Segment, Event, AnalogSignal, SpikeTrain, Unit, ChannelIndex]
    ...
    read_params = {
        Block: [
            ("nsx_to_load", {"value": "none", "label": "List of nsx files (ids, int) to read."}),
            (
                "n_starts",
                {
                    "value": None,
                    "label": "List of n_start points (Quantity) to create " "segments from.",
                },
            ),
            (
                "n_stops",
                {
                    "value": None,
                    "label": "List of n_stop points (Quantity) to create " "segments from.",
                },
            ),
            (
                "channels",
                {"value": "none", "label": "List of channels (ids, int) to load data from."},
            ),
            (
                "units",
                {
            ...
            (
                ...
                {
                    "value": False,
                    "label": "States if waveforms should be loaded and attached " "to spiketrain",
                },
            ),
            ("load_events", {"value": False, "label": "States if events should be loaded."}),
        ],
        Segment: [
            ("n_start", {"label": "Start time point (Quantity) for segment"}),
            ("n_stop", {"label": "Stop time point (Quantity) for segment"}),
            ("nsx_to_load", {"value": "none", "label": "List of nsx files (ids, int) to read."}),
            (
                "channels",
                {"value": "none", "label": "List of channels (ids, int) to load data from."},
    ...
    name = "Blackrock IO"
    description = "This IO reads .nev/.nsX file of the Blackrock " + "(Cerebus) recordings system."
    ...
    def __init__(
        ...
        warnings.warn(
            "{} is deprecated and will be removed in neo version 0.10. Use {} instead."
            "".format(self.__class__, neo.io.blackrockio.BlackrockIO),
            FutureWarning,
        )
        ...
        for ext in self.extensions:
            self.filename = re.sub(os.path.extsep + ext + "$", "", filename)

        # remove extensions from overrides
        self._filenames = {}
        if nsx_override:
            self._filenames["nsx"] = re.sub(os.path.extsep + r"ns[1,2,3,4,5,6]$", "", nsx_override)
        else:
            self._filenames["nsx"] = self.filename
        if nev_override:
            self._filenames["nev"] = re.sub(os.path.extsep + r"nev$", "", nev_override)
        else:
            self._filenames["nev"] = self.filename
        if sif_override:
            self._filenames["sif"] = re.sub(os.path.extsep + r"sif$", "", sif_override)
        else:
            self._filenames["sif"] = self.filename
        if ccf_override:
            self._filenames["ccf"] = re.sub(os.path.extsep + r"ccf$", "", ccf_override)
        else:
        ...
        for ext, ext_override in zip(exts, ext_overrides):
            if ext_override is not None and self._avail_files[ext] is False:
                raise ValueError(
                    "Specified {} file {} could not be " "found.".format(ext, ext_override)
                )
        ...
        if self._avail_files["nev"]:
            ...
            # read nev headers
            self.__nev_basic_header, self.__nev_ext_header = self.__nev_header_reader[
                self.__nev_spec
            ]()
        ...
        for nsx_nb in self._avail_nsx:
            ...
            # read nsx headers
            (
                self.__nsx_basic_header[nsx_nb],
                self.__nsx_ext_header[nsx_nb],
            ) = self.__nsx_header_reader[self.__nsx_spec[nsx_nb]](nsx_nb)

            # Read nsx data header(s) for nsx
            self.__nsx_data_header[nsx_nb] = self.__nsx_dataheader_reader[self.__nsx_spec[nsx_nb]](
                nsx_nb
            )
    ...
    def __extract_nev_file_spec(self):
        ...
        if nev_file_id["file_id"].decode() == "NEURALEV":
            ...
            raise IOError("NEV file type {} is not supported".format(nev_file_id["file_id"]))
    ...
    def __read_nsx_header_variant_a(self, nsx_nb):
        ...
        nsx_ext_header = np.memmap(filename, mode="r", shape=shape, offset=offset_dt0, dtype=dt1)
    ...
    def __read_nsx_dataheader(self, nsx_nb, offset):
        ...
        # dtypes data header
        dt2 = [("header", "uint8"), ("timestamp", "uint32"), ("nb_data_points", "uint32")]

        return np.memmap(filename, mode="r", dtype=dt2, shape=1, offset=offset)[0]
    ...
    def __read_nsx_data_variant_a(self, nsx_nb):
        ...
        data = {1: np.memmap(filename, mode="r", dtype="int16", shape=shape, offset=offset)}
    ...
    def __read_nev_header(self, ext_header_variants):
        ...
        raw_ext_header = np.memmap(filename, mode="r", offset=offset_dt0, dtype=dt1, shape=shape)
    ...
    def __nev_params(self, param_name):
        ...
        nev_parameters = {
            ...
            "event_unit": pq.CompoundUnit(
                "1.0/{} * s".format(self.__nev_basic_header["timestamp_resolution"])
            ),
            "nb_units": dict(
                zip(
            ...
            "waveform_time_unit": pq.CompoundUnit(
                "1.0/{} * s".format(self.__nev_basic_header["sample_resolution"])
            ),
            "waveform_unit": pq.uV,
        }
    ...
    def __get_nev_rec_times(self):
        ...
        last_data_packet = np.memmap(filename, mode="r", offset=offset, dtype=dt)[0]
    ...
    def __get_nsx_rec_times_variant_a(self, nsx_nb):
        ...
        t_unit = self.__nsx_params[self.__nsx_spec[nsx_nb]]("time_unit", nsx_nb)
        highest_res = self.__nev_params("event_unit")

        bytes_in_headers = self.__nsx_params[self.__nsx_spec[nsx_nb]]("bytes_in_headers", nsx_nb)
        nb_data_points = int(
    ...
    def __get_nsx_param_variant_a(self, param_name, nsx_nb):
        ...
        nsx_parameters = {
            "labels": labels,
            "units": np.array([b"uV"] * self.__nsx_basic_header[nsx_nb]["channel_count"]),
            ...
            * self.__nsx_basic_header[nsx_nb]["channel_count"],
            ...
            "time_unit": pq.CompoundUnit(
                "1.0/{}*s".format(30000 / self.__nsx_basic_header[nsx_nb]["period"])
            ),
        }
    ...
    def __get_nsx_databl_param_variant_a(self, param_name, nsx_nb, n_start=None, n_stop=None):
        """
        ...
        t_starts, t_stops = self.__nsx_rec_times[self.__nsx_spec[nsx_nb]](nsx_nb)
    ...
    def __get_nsx_databl_param_variant_b(self, param_name, nsx_nb, n_start, n_stop):
        """
        ...
        for d_bl in self.__nsx_data_header[nsx_nb].keys():
            ...
            data_parameters = {
                "nb_data_points": self.__nsx_data_header[nsx_nb][d_bl]["nb_data_points"],
                "databl_idx": d_bl,
            ...
            if t_starts[d_bl - 1] <= n_start < n_stop <= t_stops[d_bl - 1]:
                ...
                self._print_verbose(
                    ...
                    "({}).".format(
                        n_start, n_stop, nsx_nb, (t_starts[d_bl - 1], t_stops[d_bl - 1])
                    )
                )
                return data_parameters[param_name]
            else:
    ...
    def __get_unit_classification(self, un_id):
        ...
        if un_id == 0:
            ...
            raise ValueError("Unit id {} is not used by daq system".format(un_id))
        elif un_id == 255:
            return "noise"
        else:
    ...
    def __transform_channels(self, channels, nsx_to_load):
        ...
        if nsx_to_load is not None:
            for nsx_nb in nsx_to_load:
                all_channels.extend(self.__nsx_ext_header[nsx_nb]["electrode_id"].astype(int))
        ...
        if channels:
            if len(set(all_channels) & set(channels)) < len(channels):
                self._print_verbose("Ignoring unknown channel ID(s) specified in in channels.")

            # Make sure, all channels are valid and contain no duplicates
            channels = list(set(all_channels).intersection(set(channels)))
        else:
            self._print_verbose(
                "No channel is specified, therefore no " "time series and unit data is loaded."
            )
    ...
    def __transform_units(self, units, channels):
        ...
        if isinstance(units, dict):
            for ch, u in units.items():
                if ch not in channels:
                    self._print_verbose(
                        "Units contain a channel id which is not listed in " "channels"
                    )
                if isinstance(u, int):
                    units[ch] = [u]
            ...
            if (channels is None) and (units is not None):
                raise ValueError("At least one channel needs to be loaded to load units")
        ...
        if units is None:
            self._print_verbose(
                "No units are specified, therefore no " "unit or spiketrain is loaded."
            )
    ...
    def __transform_times(self, n, default_n):
        ...
        if isinstance(n, pq.Quantity):
            ...
            n = [tp.rescale(highest_res) if tp is not None else default_n for tp in n]
        elif n is None:
            n = [default_n]
        else:
    ...
    def __merge_time_ranges(self, user_n_starts, user_n_stops, nsx_to_load):
        """
        ...
        highest_res = self.__nev_params("event_unit")
        user_n_starts = self.__transform_times(user_n_starts, min_time)
        user_n_stops = self.__transform_times(user_n_stops, max_time)
        ...
        while start_stop_id < len(user_n_starts):
            ...
            if user_n_stops[start_stop_id] < min_time or user_n_starts[start_stop_id] > max_time:
        ...
        if nsx_to_load is not None:
            for nsx_nb in nsx_to_load:
                start_stop = self.__nsx_rec_times[self.__nsx_spec[nsx_nb]](nsx_nb)
        ...
        if len(n_starts_files) > 0:
            if np.shape(n_starts_files)[1] > 1:
                n_starts_files = [tp * highest_res for tp in np.min(n_starts_files, axis=1)]
            else:
                n_starts_files = [tp * highest_res for tp in np.min(n_starts_files, axis=0)]

        # reducing n_starts from wanted nsx files to maxima
        # (keep recording pause if it occurs)
        if len(n_stops_files) > 0:
            if np.shape(n_stops_files)[1] > 1:
                n_stops_files = [tp * highest_res for tp in np.max(n_stops_files, axis=1)]
            else:
                n_stops_files = [tp * highest_res for tp in np.max(n_stops_files, axis=0)]
        ...
        for start, stop in zip(user_n_starts, user_n_stops):
            ...
            if not start < stop:
                raise ValueError("t(i) in n_starts has to be smaller than t(i) in n_stops")
            ...
            if len(n_starts_files) > 0:
                ...
                merged_n_starts = [start] + [tp * highest_res for tp in red_n_starts_files]
            else:
            ...
            if len(n_stops_files) > 0:
                ...
                merged_n_stops = [tp * highest_res for tp in red_n_stops_files] + [stop]
            else:
                merged_n_stops = [stop]
    ...
    def __read_spiketrain(
        ...
        if load_waveforms and not lazy:
            ...
            if scaling == "voltage":
                st.waveforms = (
                    waveforms[mask]
                    * self.__nev_params("waveform_unit")
                ...
                raise ValueError("Unkown option {1} for parameter scaling.".format(scaling))
            st.sampling_rate = self.__nev_params("waveform_sampling_rate")
            st.left_sweep = self.__get_left_sweep_waveforms()[channel_id]

        # add additional annotations
        st.annotate(unit_id=int(unit_id), channel_id=int(channel_id))

        return st

    def __read_analogsignal(
        self, n_start, n_stop, signal, channel_id, nsx_nb, scaling="raw", lazy=False
    ):
        """
        ...
        # get parameters
        sampling_rate = self.__nsx_params[self.__nsx_spec[nsx_nb]]("sampling_rate", nsx_nb)
        nsx_time_unit = self.__nsx_params[self.__nsx_spec[nsx_nb]]("time_unit", nsx_nb)
        max_ana = self.__nsx_params[self.__nsx_spec[nsx_nb]]("max_analog_val", nsx_nb)
        min_ana = self.__nsx_params[self.__nsx_spec[nsx_nb]]("min_analog_val", nsx_nb)
        max_dig = self.__nsx_params[self.__nsx_spec[nsx_nb]]("max_digital_val", nsx_nb)
        min_dig = self.__nsx_params[self.__nsx_spec[nsx_nb]]("min_digital_val", nsx_nb)
        units = self.__nsx_params[self.__nsx_spec[nsx_nb]]("units", nsx_nb)
        labels = self.__nsx_params[self.__nsx_spec[nsx_nb]]("labels", nsx_nb)
        ...
        description = "AnalogSignal from channel: {}, label: {}, nsx: {}".format(
            channel_id, labels[idx_ch], nsx_nb
        )

        # TODO: Find a more time/memory efficient way to handle lazy loading
        data_times = (
            np.arange(t_start.item(), t_stop.item(), self.__nsx_basic_header[nsx_nb]["period"])
            * t_start.units
        )
        ...
        if lazy:
            ...
            if scaling == "voltage":
                ...
                if sym_ana and sym_dig:
                    ...
                    sig_ch *= float(max_ana[idx_ch] - min_ana[idx_ch]) / float(
                        max_dig[idx_ch] - min_dig[idx_ch]
                    )
        ...
        anasig.annotate(nsx=nsx_nb, channel_id=int(channel_id))
        return anasig
    ...
    def __read_unit(self, unit_id, channel_id):
        ...
        un = Unit(
            name=name, description=desc, file_origin=".".join([self._filenames["nev"], "nev"])
        )

        # add additional annotations
        un.annotate(unit_id=int(unit_id), channel_id=int(channel_id))

        return un

    def __read_channelindex(self, channel_id, index=None, channel_units=None, cascade=True):
        """
        ...
        chidx = ChannelIndex(np.array([channel_id]), file_origin=self.filename)
        ...
        if self._avail_files["nev"]:
            ...
            if channel_id in self.__nev_ext_header[b"NEUEVWAV"]["electrode_id"]:
                get_idx = list(self.__nev_ext_header[b"NEUEVWAV"]["electrode_id"]).index(
                    channel_id
                )
                chidx.annotate(
                    connector_ID=self.__nev_ext_header[b"NEUEVWAV"]["physical_connector"][get_idx],
                    connector_pinID=self.__nev_ext_header[b"NEUEVWAV"]["connector_pin"][get_idx],
                    nev_dig_factor=self.__nev_ext_header[b"NEUEVWAV"]["digitization_factor"][
                        get_idx
                    ],
                    nev_energy_threshold=self.__nev_ext_header[b"NEUEVWAV"]["energy_threshold"][
                        get_idx
                    ]
                    * pq.uV,
                    nev_hi_threshold=self.__nev_ext_header[b"NEUEVWAV"]["hi_threshold"][get_idx]
                    * pq.uV,
                    nev_lo_threshold=self.__nev_ext_header[b"NEUEVWAV"]["lo_threshold"][get_idx]
                    * pq.uV,
                    nb_sorted_units=self.__nev_ext_header[b"NEUEVWAV"]["nb_sorted_units"][get_idx],
                    waveform_size=self.__waveform_size[self.__nev_spec]()[channel_id]
                    * self.__nev_params("waveform_time_unit"),
                )

                # additional annotations from nev (only for file_spec > 2.1)
                if self.__nev_spec in ["2.2", "2.3"]:
                    get_idx = list(self.__nev_ext_header[b"NEUEVFLT"]["electrode_id"]).index(
                        channel_id
                    )
                    # filter type codes (extracted from blackrock manual)
                    chidx.annotate(
                        nev_hi_freq_corner=self.__nev_ext_header[b"NEUEVFLT"]["hi_freq_corner"][
                            get_idx
                        ]
                        / 1000.0
                        * pq.Hz,
                        nev_hi_freq_order=self.__nev_ext_header[b"NEUEVFLT"]["hi_freq_order"][
                            get_idx
                        ],
                        nev_hi_freq_type=flt_type[
                            self.__nev_ext_header[b"NEUEVFLT"]["hi_freq_type"][get_idx]
                        ],
                        nev_lo_freq_corner=self.__nev_ext_header[b"NEUEVFLT"]["lo_freq_corner"][
                            get_idx
                        ]
                        / 1000.0
                        * pq.Hz,
                        nev_lo_freq_order=self.__nev_ext_header[b"NEUEVFLT"]["lo_freq_order"][
                            get_idx
                        ],
                        nev_lo_freq_type=flt_type[
                            self.__nev_ext_header[b"NEUEVFLT"]["lo_freq_type"][get_idx]
                        ],
                    )
        ...
        if self.__nev_spec in ["2.2", "2.3"] and self.__nsx_ext_header:
            ...
            if channel_id in self.__nsx_ext_header[k]["electrode_id"]:
                get_idx = list(self.__nsx_ext_header[k]["electrode_id"]).index(channel_id)
                chidx.annotate(
                    nsx_hi_freq_corner=self.__nsx_ext_header[k]["hi_freq_corner"][get_idx]
                    / 1000.0
                    * pq.Hz,
                    nsx_lo_freq_corner=self.__nsx_ext_header[k]["lo_freq_corner"][get_idx]
                    / 1000.0
                    * pq.Hz,
                    nsx_hi_freq_order=self.__nsx_ext_header[k]["hi_freq_order"][get_idx],
                    nsx_lo_freq_order=self.__nsx_ext_header[k]["lo_freq_order"][get_idx],
                    nsx_hi_freq_type=flt_type[self.__nsx_ext_header[k]["hi_freq_type"][get_idx]],
                    nsx_lo_freq_type=flt_type[self.__nsx_ext_header[k]["hi_freq_type"][get_idx]],
                )
            ...
            if channel_units is not None:
                ...
                for un_id in channel_units:
                    if un_id in np.unique(data_ch["unit_class_nb"]):
                        un = self.__read_unit(unit_id=un_id, channel_id=channel_id)
    ...
    def read_segment(
        ...
        if self._avail_files["nev"]:
            ...
            if load_events:
                ev_dict = self.__nonneural_evtypes[self.__nev_spec](nev_data["NonNeural"])
            ...
            if units is not None:
                ...
                for ch_id in units.keys():
                    ...
                    if units[ch_id] is not None:
                        ...
                        self._print_verbose(
                            "There are no units specified for channel " "{}".format(ch_id)
                        )

        if nsx_to_load is not None:
            for nsx_nb in nsx_to_load:
                # read nsx data
                nsx_data = self.__nsx_data_reader[self.__nsx_spec[nsx_nb]](nsx_nb)
    ...
    def read_block(
        ...
        bl.annotate(avail_file_set=[k for k, v in self._avail_files.items() if v])
        ...
        n_starts, n_stops = self.__merge_time_ranges(n_starts, n_stops, nsx_to_load)
        ...
        if channels:
            for ch_id in channels:
                ...
                chidx = self.__read_channelindex(
                    channel_id=ch_id, index=0, channel_units=ch_units, cascade=cascade
                )

                for seg in bl.segments:
                    if ch_units:
                        for un in chidx.units:
                            sts = seg.filter(targdict={"name": un.name}, objects="SpikeTrain")
                            for st in sts:
                                un.spiketrains.append(st)

                    anasigs = seg.filter(targdict={"channel_id": ch_id}, objects="AnalogSignal")
    ...
    def __str__(self):
        ...
        if self._avail_files["nev"]:
            ...
            if b"NEUEVWAV" in self.__nev_ext_header.keys():
                avail_el = self.__nev_ext_header[b"NEUEVWAV"]["electrode_id"]
                con = self.__nev_ext_header[b"NEUEVWAV"]["physical_connector"]
                pin = self.__nev_ext_header[b"NEUEVWAV"]["connector_pin"]
                nb_units = self.__nev_ext_header[b"NEUEVWAV"]["nb_sorted_units"]
                ...
                for i, el in enumerate(avail_el):
                    ...
                    output += "nb_units: %i\n" % nb_units[i]
        for nsx_nb in self._avail_nsx:
            analog_res = self.__nsx_params[self.__nsx_spec[nsx_nb]]("sampling_rate", nsx_nb)
            avail_el = [el for el in self.__nsx_ext_header[nsx_nb]["electrode_id"]]
            output += (
