...
from neo.core import Block, Event, Group, Segment, SpikeTrain, Unit
...
class BrainwareSrcIO(BaseIO):
    ...
    def read_next_block(self, **kargs):
        ...
        self._unit0 = Group(
            name="UnassignedSpikes", elliptic=[], boundaries=[], timestamp=[], max_valid=[]
        )
    ...
    def _assign_sequence(self, data_obj):
        ...
        if isinstance(data_obj, Group):
            ...
            if self.logger.isEnabledFor(logging.WARNING):
                self.logger.warning(
                    "Unrecognized sequence of type %s found, " "skipping", type(data_obj)
                )
    ...
    def _combine_spiketrains(self, spiketrains):
        ...
        if hasattr(spiketrains[0], "t_stop"):
            ...
            trig2 = np.array(
                np.concatenate([itrain.annotations["trig2"] for itrain in spiketrains], axis=1)
            )
    ...
    def __read_comment(self):
        ...
        comment = Event(
            times=pq.Quantity(time, units=pq.d),
            labels=[text],
            sender=sender,
            file_origin=self._file_origin,
        )
    ...
    def __read_segment(self):
        ...
        spiketrainlen = pq.Quantity(
            np.fromfile(self._fsrc, dtype=np.int32, count=1)[0], units=pq.ms, copy=False
        )
    ...
    def __read_segment_list_var(self):
        ...
        sampling_period = pq.Quantity(
            np.fromfile(self._fsrc, dtype=np.float32, count=1), units=pq.us, copy=False
        )[0]
...
if __name__ == "__main__":
    ...
    for path in get_test_file_full_path(
        ioclass=BrainwareSrcIO, filename=FILES_TO_TEST, directory=local_test_dir
    ):
