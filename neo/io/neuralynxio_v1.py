...
from neo.core import Block, Segment, ChannelIndex, AnalogSignal, SpikeTrain, Event, Unit
...
class NeuralynxIO(BaseIO):
    ...
    name = "Neuralynx"
    description = (
        "This IO reads .nse/.ncs/.nev files of the Neuralynx ("
    ...
    def __init__(
        ...
        warnings.warn(
            ...
            "".format(self.__class__, neo.io.neuralynxio.NeuralynxIO),
            FutureWarning,
        )
        ...
        if sessiondir is None:
            raise ValueError(
                "Must provide a directory containing data files of" " of one recording session."
            )
    ...
    def read_block(
        ...
        for gap in self.parameters_global["gaps"]:
            ...
            for e in range(len(t_starts)):
                ...
                if (
                    ...
                    or (
                        t1 is None and t2 is not None and t2.rescale(self.ncs_time_unit) > gap_stop
                    )
                    or (
                        t2 is None and t1 is not None and t1.rescale(self.ncs_time_unit) < gap_stop
                    )
                    or (
                        t1 is not None
                        and t2 is not None
                        and t1.rescale(self.ncs_time_unit) < gap_start
        ...
        for anasig in [a for seg in bl.segments for a in seg.analogsignals]:
            ...
            channelidx = ChannelIndex(
                index=range(len(channelids)),
                channel_names=channel_names,
                name="channel ids for all analogsignal " '"%s"' % anasig.name,
        ...
        channelids = [unit.spiketrains[0].annotations["channel_index"] for unit in units]
        channel_names = ["channel %i" % i for i in channelids]
        channelidx = ChannelIndex(
            index=range(len(channelids)),
            channel_names=channel_names,
    ...
    def read_segment(
        ...
        if electrode_list == [] or electrode_list is None:
            ...
            warnings.warn(
                "Requested channels %s are not present in session "
        ...
        if analogsignals:
            for chid in electrode_list:
                if chid in self.parameters_ncs:
                    ...
                    self.read_ncs(
                        file_ncs, empty_seg, lazy, cascade, t_start=t_start, t_stop=t_stop
                    )
                else:
                    self._diagnostic_print(
                        "Can not load ncs of channel %i. "
    ...
    def read_ncs(self, filename_ncs, seg, lazy=False, cascade=True, t_start=None, t_stop=None):
        ...
        if chid is None:
            raise ValueError(
                "NeuralynxIO is attempting to read a file "
        ...
        if lazy:
            ...
            if len(starts) == 0:
                self._diagnostic_print(
                    "Requested AnalogSignal not present in this time " "interval."
                )
                return
            else:
            ...
            if "broken_packet" in self.parameters_ncs[chid]:
                broken_packets = [
                    packet[0]
                    for packet in self.parameters_ncs[chid]["broken_packet"]
        ...
        anasig = AnalogSignal(
            signal=pq.Quantity(sig, unit, copy=False),
            sampling_rate=1 * sampling_rate,
            # rescaling t_start to sampling time units
            t_start=(
                header_time_data[p_id_start] * self.ncs_time_unit
                - self.parameters_global["t_start"]
            ).rescale(1 / sampling_rate),
        ...
        for event_type in self.parameters_nev[filename_nev]["event_types"]:
            ...
            type_mask = [
                ...
                if (
                    ...
                    and data[i][10].decode("latin-1") == event_type["name"]
                )
            ]
            marker_times = [
                t[3] for t in data[type_mask]
            ...
            time_mask = [
                ...
                if (marker_times[i] >= t_start and marker_times[i] <= t_stop)
            ]
            ...
            ev = Event(
                times=pq.Quantity(marker_times, units=self.nev_time_unit, dtype="int"),
    ...
    def read_nse(
        ...
        if channel_id is not None:
            ...
            if filename_nse in self.nse_avail:
                warnings.warn(
                    "NeuralynxIO is attempting to read an empty "
        ...
        # reading data
        [timestamps, channel_ids, cell_numbers, features, data_points] = self.__mmap_nse_packets(
            filename_nse
        )
    ...
    def read_ntt(
        ...
        # reading data
        [timestamps, channel_ids, cell_numbers, features, data_points] = self.__mmap_ntt_packets(
            filename_ntt
        )
        ...
        for unit_i in unit_list:
            if not lazy:
                ...
                spike_times = spike_times[
                    np.where(np.logical_and(spike_times >= t_start, spike_times < t_stop))
                ]
            else:
    ...
    def _associate(self, cachedir=None, usecache="hash"):
        ...
        if self.associated:
            raise OSError("Trying to associate an already associated NeuralynxIO " "object.")
        ...
        self.sessionfiles = [
            f for f in listdir(self.sessiondir) if isfile(os.path.join(self.sessiondir, f))
        ]
        ...
        if cachedir is not None and usecache != "never":
            ...
            if usecache == "hash":
                ...
                for f in self.sessionfiles:
                    file_hash = self.hashfile(
                        open(self.sessiondir + sep + f, "rb"), hashlib.sha256()
                    )
                    hashes_calc[f] = file_hash
            elif usecache == "datesize":
                hashes_calc = {}
                for f in self.sessionfiles:
                    hashes_calc[f] = self.datesizefile(self.sessiondir + sep + f)
            ...
            if usecache == "always" or all(
                [
                    f in hashes_calc and f in hashes_read and hashes_calc[f] == hashes_read[f]
                    for f in self.sessionfiles
                ]
            ):
                check_files = False
                ...
                parameterfile = (
                    cachedir + sep + self.sessiondir.split(sep)[-1] + "/parameters.cache"
                )
                ...
                for IOdict, dictname in [
                    (self.parameters_global, "global"),
        ...
        if check_files:
            ...
            for parameter_collection in [
                ...
                if any(
                    np.abs(np.diff([i["recording_opened"] for i in parameter_collection.values()]))
            ...
            parameter_collection = (
                list(self.parameters_ncs.values())
            ...
            for file_type, parameter_collection in [
                ...
                if (
                    ...
                    and len(
                        np.unique(
                            np.array([i[name_t1].magnitude for i in parameter_collection.values()])
                        )
                    )
                    > 1
                ):
                ...
                if len([i[name_t1] for i in parameter_collection.values()]):
                    self.parameters_global["%s_t_start" % file_type] = min(
                        [i[name_t1] for i in parameter_collection.values()]
                    )
                    self.parameters_global["%s_t_stop" % file_type] = min(
                        [i[name_t2] for i in parameter_collection.values()]
                    )
            ...
            for g in range(len(list(self.parameters_ncs.values())[0]["gaps"])):
                integrated = False
                gap_stats = np.unique(
                    [i["gaps"][g] for i in self.parameters_ncs.values()], return_counts=True
                )
                if len(gap_stats[0]) != 3 or len(np.unique(gap_stats[1])) != 1:
                    ...
                    for gg in range(len(self.parameters_global["gaps"])):
                        ...
                        #  -> continuous gap
                        if globalgap[2] == list(self.parameters_ncs.values())[0]["gaps"][g][1]:
                            self.parameters_global["gaps"][gg] = self.parameters_global["gaps"][
                                gg
                            ][:2] + (list(self.parameters_ncs.values())[0]["gaps"][g][2],)
                            integrated = True
                            break
                    ...
                    if not integrated:
                        ...
                        self.parameters_global["gaps"].append(
                            list(self.parameters_ncs.values())[0]["gaps"][g]
                        )
        ...
        if cachedir is not None and usecache != "never":
            ...
            if usecache != "always":
                pickle.dump(
                    hashes_calc,
                    open(cachedir + sep + self.sessiondir.split(sep)[-1] + "/hashkeys", "wb"),
                )
    ...
    def __mmap_nse_packets(self, filename):
        ...
        if filesize > 16384:
            ...
            timestamps = (
                data[:, 0] + data[:, 1] * 2 ** 16 + data[:, 2] * 2 ** 32 + data[:, 3] * 2 ** 48
            )
    ...
    def __mmap_nev_file(self, filename):
        ...
        if getsize(self.sessiondir + sep + filename) > 16384:
            return np.memmap(
                self.sessiondir + sep + filename, dtype=nev_dtype, mode="r", offset=16384
            )
        else:
            return None
    ...
    def __mmap_ntt_file(self, filename):
        ...
        if getsize(self.sessiondir + sep + filename) > 16384:
            return np.memmap(
                self.sessiondir + sep + filename, dtype=nse_dtype, mode="r", offset=16384
            )
        else:
            return None
    ...
    def __read_text_header(self, filename, parameter_dict):
        ...
        text_header = codecs.open(self.sessiondir + sep + filename, "r", "latin-1").read(16384)

        parameter_dict["cheetah_version"] = self.__get_cheetah_version_from_txt_header(
            text_header, filename
        )

        parameter_dict.update(
            self.__get_filename_and_times_from_txt_header(
    ...
    def __get_cheetah_version_from_txt_header(self, text_header, filename):
        ...
        if match:
            ...
            raise ValueError(
                "Can not extract Cheetah version from file " "header of file %s" % filename
            )

    def __get_filename_and_times_from_txt_header(self, text_header, version):
        if parse_version(version) <= parse_version("5.6.4"):
            datetime1_regex = re.compile(
                r"## Time Opened \(m/d/y\): " r"(?P<date>\S+)" r"  \(h:m:s\.ms\) " r"(?P<time>\S+)"
            )
            datetime2_regex = re.compile(
                r"## Time Closed \(m/d/y\): " r"(?P<date>\S+)" r"  \(h:m:s\.ms\) " r"(?P<time>\S+)"
            )
            ...
            datetime1_regex = re.compile(r"-TimeCreated " r"(?P<date>\S+) " r"(?P<time>\S+)")
            datetime2_regex = re.compile(r"-TimeClosed " r"(?P<date>\S+) " r"(?P<time>\S+)")
            filename_regex = re.compile(r"-OriginalFileName " r'"?(?P<filename>\S+)"?')
        ...
        if "## Time Closed File was not closed properly" in text_header:
            warnings.warn(
                "Text header of file %s does not contain recording "
                "closed time. File was not closed properly."
                "" % filename
            )

        datetime1 = datetime.datetime.strptime(
            matchtime1["date"] + " " + matchtime1["time"], datetimeformat
        )
        datetime2 = datetime.datetime.strptime(
            matchtime2["date"] + " " + matchtime2["time"], datetimeformat
        )

        output = {
            "recording_opened": datetime1,
    ...
    def __read_nev_data_header(self, filehandle, filename):
        ...
        if filename in self.parameters_nev:
            ...
            if "Starting_Recording" in self.parameters_nev[filename]:
                raise ValueError(
                    "Trying to read second nev file of name %s. "
            ...
            self.parameters_nev[filename]["t_stop"] = max(
                [e["timestamp"] for e in self.parameters_nev[filename]["events"]]
            )
            ...
            for d in event_types:
                d.pop("timestamp")
            self.parameters_nev[filename]["event_types"] = [
                dict(y) for y in {tuple(x.items()) for x in event_types}
            ]
    ...
    def __ncs_gap_check(self, filehandle):
        ...
        packet_checks = (
            valid_samples / (self.ncs_time_unit.rescale(pq.s).magnitude * sampling_rate)
        ) == data_packet_offsets
        if not all(packet_checks):
            ...
            for broken_packet in broken_packets:
                ...
                self._diagnostic_print(
                    "Detected broken packet in NCS file at "
        ...
        if not all(data_packet_offsets == delta_t):
            ...
            for gap_packet_id in gap_packet_ids:
                ...
                if (
                    gap_packet_id in self.parameters_ncs[channel_id]["invalid_first_samples"]
                    or gap_packet_id + 1
                    in self.parameters_ncs[channel_id]["invalid_first_samples"]
                ):
                    continue
                ...
                self.parameters_ncs[channel_id]["gaps"].append(
                    (gap_packet_id, gap_start, gap_stop)
                )  #
                #  [,microsec,microsec]
                self._diagnostic_print(
                    "Detected gap in NCS file between"
                    "sample time %i and %i  (last correct "
                    "packet id %i)" % (gap_start, gap_stop, gap_packet_id)
                )
    ...
    def __ncs_invalid_first_sample_check(self, filehandle):
        ...
        if len(invalid_packet_ids) > 0:
            warnings.warn(
                "Invalid first sample(s) detected in ncs file"
            ...
            for invalid_packet_id in invalid_packet_ids:
                if invalid_packet_id < 2 or invalid_packet_id > len(filehandle) - 2:
                    raise ValueError(
                        "Corrupted ncs data packet at the beginning" "or end of file."
                    )
                elif timestamps[invalid_packet_id + 1] - timestamps[invalid_packet_id - 1] != 2 * (
                    timestamps[invalid_packet_id - 1] - timestamps[invalid_packet_id - 2]
                ):
                    raise ValueError(
                        "Starting times of ncs data packets around"
    ...
    def get_channel_id_by_file_name(self, filename):
        ...
        channel_ids = []
        channel_ids += [
            k for k in self.parameters_ncs if self.parameters_ncs[k]["filename"] == filename
        ]
        channel_ids += [
            k for k in self.parameters_nse if self.parameters_nse[k]["filename"] == filename
        ]
        channel_ids += [
            k for k in self.parameters_ntt if self.parameters_ntt[k]["filename"] == filename
        ]
