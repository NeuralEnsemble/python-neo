...
class ElanIO(BaseIO):
    ...
    def read_segment(self, lazy=False, cascade=True):
        ...
        for c in range(nbchannel):
            ...
            try:
                unit = un.Quantity(1, units[c])
            except:
                unit = un.Quantity(1, '')

            ana_sig = AnalogSignal(
                sig * unit, sampling_rate=sampling_rate,
                t_start=0. * un.s, name=labels[c], channel_index=c)
            if lazy:
        ...
        #~ # so auto translate
        #~ if anaSig.units == un.V or anaSig.units == un.mV:
        #~ s = anaSig.rescale('uV').magnitude
        #~ elif anaSig.units == un.uV:
        #~ s = anaSig.magnitude
        #~ else:
