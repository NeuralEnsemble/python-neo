...
class NeurosharectypesIO(BaseIO):
    ...
    def read_segment(self, import_neuroshare_segment=True, lazy=False):
        ...
        seg.annotate(
            neuroshare_version=str(info.dwAPIVersionMaj) + "." + str(info.dwAPIVersionMin)
        )
        ...
        for dwEntityID in range(fileinfo.dwEntityCount):
            ...
            neuroshare.ns_GetEntityInfo(
                hFile, dwEntityID, ctypes.byref(entityInfo), ctypes.sizeof(entityInfo)
            )
            ...
            if entity_types[entityInfo.dwEntityType] == "ns_ENTITY_EVENT":
                ...
                neuroshare.ns_GetEventInfo(
                    hFile, dwEntityID, ctypes.byref(pEventInfo), ctypes.sizeof(pEventInfo)
                )
            ...
            if entity_types[entityInfo.dwEntityType] == "ns_ENTITY_ANALOG":
                ...
                neuroshare.ns_GetAnalogInfo(
                    hFile, dwEntityID, ctypes.byref(pAnalogInfo), ctypes.sizeof(pAnalogInfo)
                )
            ...
            # segment
            if (
                entity_types[entityInfo.dwEntityType] == "ns_ENTITY_SEGMENT"
                and import_neuroshare_segment
            ):
                ...
                for dwIndex in range(entityInfo.dwItemCount):
                    ...
                    waveforms[dwIndex, :, :] = (
                        pData[: nsample * nsource].reshape(nsample, nsource).transpose()
                    )
                ...
                sptr = SpikeTrain(
                    ...
                    waveforms=pq.Quantity(
                        waveforms, units=str(pdwSegmentInfo.szUnits), copy=False
                    ),
            ...
            if entity_types[entityInfo.dwEntityType] == "ns_ENTITY_NEURALEVENT":
                ...
                neuroshare.ns_GetNeuralInfo(
                    hFile, dwEntityID, ctypes.byref(pNeuralInfo), ctypes.sizeof(pNeuralInfo)
                )
                ...
                sptr = SpikeTrain(times, t_stop=t_stop, name=str(entityInfo.szEntityLabel),)
                seg.spiketrains.append(sptr)
