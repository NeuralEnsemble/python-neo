...
class KwikIO(BaseIO):
    ...
    def __init__(self, filename) :
        ...
        self._kwd = h5py.File(basename + '.raw.kwd', 'r')
    ...
    def read_segment(self,
        ...
        """
        Channel_index can be int, iterable or None to select all
        """

        raw_shape = self._kwd['recordings'][str(dataset)]['data'].shape # Do we need this?
        attrs = self._kwd['recordings'][str(dataset)].attrs
        # create an empty segment
        seg = Segment( name='session something' )

        if cascade:
            # read nested analosignal
            ana = self._read_traces(lazy=lazy,
                                    cascade=cascade,
                                    dataset=dataset,
            ...
            seg.duration = (raw_shape[0] #TODO: this duration is not necessarily correct after downsample
                          / attrs['sample_rate']
                          + attrs['start_time']) * pq.s
    ...
    def _read_traces(self,
        ...
        """
        raw_shape = self._kwd['recordings'][str(dataset)]['data'].shape
        attrs = self._kwd['recordings'][str(dataset)].attrs

        if sampling_rate:
            sliceskip = int(attrs['sample_rate']/sampling_rate)
        else:
            sliceskip = 1
            sampling_rate = attrs['sample_rate']

        if channel_index is not None:
            if type(channel_index) is int: channel_index = [channel_index]
        else:
            channel_index = range(0,raw_shape[1])
        ...
        if lazy:
            ...
            anasig.lazy_shape = raw_shape #TODO: wrong if downsampled
        else:
            data_array = []
            for idx in channel_index:
                data = self._kwd['recordings'][str(dataset)]['data'].value[:,idx]
                data_array.append(data[0:-1:sliceskip])
            data = self._bits2volt(np.array(data_array).T)
