...
class BaseFromRaw(BaseIO):
    ...
    supported_objects = [Block, Segment, AnalogSignal, SpikeTrain, Unit, Group, Event, Epoch]
        ...
        if create_group_across_segment["AnalogSignal"]:
            ...
            for channel_index in channel_indexes_list:
                for i, (ind_within, ind_abs) in self._make_signal_channel_subgroups(
                    channel_index, signal_group_mode=signal_group_mode
                ).items():
    ...
    def read_segment(
        ...
        if signal_channels.size > 0:
            ...
            for channel_indexes in channel_indexes_list:
                for i, (ind_within, ind_abs) in self._make_signal_channel_subgroups(
                    channel_indexes, signal_group_mode=signal_group_mode
                ).items():
                    # make a proxy...
        ...
        for unit_index in range(len(unit_channels)):
            ...
            sptr = SpikeTrainProxy(
                rawio=self, unit_index=unit_index, block_index=block_index, seg_index=seg_index
            )
    ...
    def _make_signal_channel_subgroups(
        self, channel_indexes, signal_group_mode="group-by-same-units"
    ):
        """
