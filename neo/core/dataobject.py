    ...
    # If array annotation, check for correct length, only single dimension and allowed data
        ...
        def _check_single_elem(element):
            ...
            if isinstance(element, list) or (
                ...
                and not (
                    isinstance(element, pq.Quantity)
                    and (element.shape == () or element.shape == (1,))
                )
            ):
        ...
        # In case of list, it needs to be ensured that all data are of the same type
            ...
            if value.dtype == object:
                raise ValueError(
                    "Cannot convert list of incompatible types into a single" " array annotation"
                )
...
class DataObject(BaseNeo, pq.Quantity):
    ...
    def __init__(
        self, name=None, description=None, file_origin=None, array_annotations=None, **annotations
    ):
        """
        ...
        BaseNeo.__init__(
            self, name=name, description=description, file_origin=file_origin, **annotations
        )
    ...
    def _merge_array_annotations(self, other):
        ...
        omitted_keys_other = [
            key for key in other.array_annotations if key not in self.array_annotations
        ]
