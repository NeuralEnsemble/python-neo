    ...
    *Optional attributes/properties*:
    ...
    def rescale(self, units):
        ...
        obj = self.duplicate_with_new_data(
            times=self.view(pq.Quantity).rescale(dim), labels=self.labels, units=units
        )
    ...
    def set_labels(self, labels):
        if self.labels is not None and self.labels.size > 0 and len(labels) != self.size:
            raise ValueError(
                "Labels array has different length to times ({} != {})".format(
                    len(labels), self.size
                )
            )
    ...
    def to_epoch(self, pairwise=False, durations=None):
        ...
        if pairwise:
            ...
            if durations is not None:
                raise ValueError(
                    "Inconsistent arguments. " "Cannot give both `pairwise` and `durations`"
                )
            if self.size % 2 != 0:
                raise ValueError(
                    "Pairwise conversion of events to epochs" " requires an even number of events"
                )
            times = self.times[::2]
