...
def check_has_dimensions_time(*values):
    ...
    for value in values:
        ...
        if (
            len(dim) != 1
            or list(dim.values())[0] != 1
            or not isinstance(list(dim.keys())[0], pq.UnitTime)
        ):
            errmsgs.append("value {} has dimensions {}, not [time]".format(value, dim))
    if errmsgs:
        raise ValueError("\n".join(errmsgs))
    ...
    *Slicing*:
    ...
    def __setitem__(self, i, value):
        ...
        if not hasattr(value, "units"):
            ...
            )  # or should we be strict: raise ValueError(
    ...
    def duplicate_with_new_data(
        self, signal, t_start=None, t_stop=None, waveforms=None, deep_copy=True, units=None
    ):
        """
        ...
        new_st = self.__class__(
            signal, t_start=t_start, t_stop=t_stop, waveforms=waveforms, units=units
        )
    ...
    def time_slice(self, t_start, t_stop):
        ...
        if _t_start > self.t_stop or _t_stop < self.t_start:
            ...
            raise ValueError(
                "A time slice completely outside the "
    ...
    def merge(self, *others):
        ...
        for other in others:
            if isinstance(other, neo.io.proxyobjects.SpikeTrainProxy):
                ...
                raise MergeError(
                    "Cannot merge, only SpikeTrain" "can be merged into a SpikeTrain."
                )
            ...
            if self.segment != other.segment:
                raise MergeError(
                    "Cannot merge these signals as they belong to" " different segments."
                )
        ...
        if all(wfs):
            wfs_stack = np.vstack(
                [st.waveforms.rescale(self.waveforms.units) for st in all_spiketrains]
            )
    ...
    def _merge_array_annotations(self, others, sorting=None):
        ...
        for key in keys:
            try:
                ...
                other_ann = np.concatenate(
                    [deepcopy(other.array_annotations[key]) for other in others]
                )
        ...
        if omitted_keys_self or omitted_keys_other:
            warnings.warn(
                ...
                "".format(omitted_keys_self, omitted_keys_other),
                UserWarning,
            )
