...
def generate_diagram(rect_pos, rect_width, figsize):
    ...
    for name, pos in rect_pos.items():
        ...
        rect = Rectangle(pos2, rect_width, line_heigth * 1.5, facecolor="g", edgecolor="k", alpha=0.5, linewidth=2.0)
        ax.add_patch(rect)
        ...
        # necessary attr
        pos2 = pos[1] + htotal - line_heigth * (1.5 + len(allrelationship) + len(obj._necessary_attrs))
        rect = Rectangle(
            (pos[0], pos2), rect_width, line_heigth * len(obj._necessary_attrs), facecolor="r", edgecolor="k", alpha=0.5
        )
        ax.add_patch(rect)
...
def generate_diagram_simple():
    ...
    rect_pos = {
        ...
        "Group": (0.5 + rw * bf * 1, 6.5),
        #  col 2 : not do for now too complicated with our object generator
        # 'ChannelView': (.5 + rw * bf * 2, 5),
        # col 2.5
        "ImageSequence": (0.5 + rw * bf * 2.5, 3.0),
        "SpikeTrain": (0.5 + rw * bf * 2.5, 0.5),
        #  col 3
        "IrregularlySampledSignal": (0.5 + rw * bf * 3, 9),
        "AnalogSignal": (0.5 + rw * bf * 3, 7.0),
        #  col 3
